...
模板版本 9 增加独立批次重试及耗尽后的手动继续入口；版本 8 的窗口和版本 7 的报告验收策略保持。已证明终止的失败只影响对应批次，成功结果不重做，其他独立批次继续。新代次拥有独立会话、候选和不可变旧记录；手动重试不重置总预算或停止证明。完整恢复、参数查询与预检规则由[文档模板合同](document-template-contract.md#批次失败与手动重试)持有。
