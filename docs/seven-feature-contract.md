...
远程认证失败或查询超时时保留本地分支选项，并展示已分类的安全原因。用户明确选择本地分支后，确认与采集只校验该本地引用并从本地仓库冻结历史，不依赖远程发现成功；不会自动将远程选择替换为本地分支。HTTP 凭据须由启动 Loopper 的系统用户提供给命令行 Git，IDE 的登录状态不作为凭据可用证明。超时诊断区分 Git 操作名和等待上限，不记录含凭据的 URL 或原始 stderr。

项目为 Git 子目录时，本地采集来源是所属仓库，冻结的历史证据仅包含项目内变更，文件及补丁路径均相对于项目目录。快照持久化项目范围，恢复沿用冻结范围；原有未记录子目录范围的快照保持原范围。
...
单次 Git 输出上限 4,000,000 字节，总补丁上限 64,000,000 字符、100,000 个提交；历史 V1–V6 的最终 Judge 提示沿用 128 KiB UTF-8 上限；V7 不生成最终 Judge 提示。超限明确阻断，不能静默截断后报告完整成功。浅克隆来源和已保存的浅快照均不能证明完整历史，须补齐来源后重新发起。补丁切片保留原始行号；两父合并的 before blob 为 Git 重建的冲突基线，不能冒充第一父提交的文件。
...
模板 V5–V7 的报告分析使用 `TEMPLATE_ANALYSIS_CANDIDATE_NO_TOOLS`，只接收嵌入的冻结证据，仅开放当前托管代际的 `submit_template_analysis` 私有 MCP 工具；文件、命令、question 和用户 MCP 均禁用。新任务要求托管私有 MCP 已就绪，不静默退回最终文本。历史 V1–V4 仍按 `TEMPLATE_ANALYSIS_NO_TOOLS` 的冻结 JSON 文本协议恢复，历史已绑定任务的统计命令维持独立守卫。模板会话不能通过普通摘要、分叉或回退接口改变冻结分析。

模板 MCP 请求携带批次 `runId`、`idempotencyKey`、`expectedSubmissionRevision` 与完整 `candidate`。服务端校验批次/Task/Attempt/Session 与当前托管代际，严格解析身份、片段覆盖、字段和问题行号；每次新提交的回执追加保存在 V80 表，同键同内容返回原回执，同键异内容拒绝，旧 revision 返回当前 revision。候选不合法时返回 `isError=true`、`FIX_AND_RESUBMIT` 和具体问题，批次保持运行，模型在同一会话纠正，不消耗新的 Task/Stage 尝试。MCP 纠正不设固定次数上限，仍受已启用的冻结时限、预算、取消和重复工具调用守卫约束。候选接受仅保存结果，必须独立确认远端终态或取得正向停止证明后才能验证批次和汇总报告；最终文本、工具调用成功或候选回执不能代替停止证明。没有已接受的工具提交时，即使最终文本包含有效 JSON 也不生成报告。停止/取消后的新候选与旧代际提交不得推进任务；同代际既有回执允许精确重放，不能产生新的状态变化。任务删除在同一事务内先显式删除候选回执，再删除批次；失败回滚必须恢复两者。

模板 V6/V7 对明确的 `finish=length` 增加原会话续接：只在当前精确请求已结束、没有已接受 MCP 候选时准备下一条请求；正常最终文本、模型错误、RETRY、忙碌或未知状态都不能触发续接。V1–V5 保持冻结行为。V81 追加保存每次续接的前一请求、下一请求、确定性消息 ID/哈希、不同内容提交数和连续停滞计数，与批次 `RUNNING → DISPATCHING` 审计及当前请求指针在短事务中原子提交。下一检查点仍经过任务时限及模型调用预算守卫；发送前精确查询消息，已送达且哈希一致只恢复轮询，明确不存在才复核上一请求停止证明后发送，未知或内容不一致不得重发。当前轮询绑定续接消息，不得把旧轮的 length 误当成新轮完成。收到迟到的接受回执后，若续接尚未发送则先取得停止证明并验证候选，不再调用模型；停止证明未知保持阻断。

没有新的不同内容 MCP 提交时，连续三次长度耗尽停止自动续接（初次截断后至多两次续接），显示 `TEMPLATE_ANALYSIS_STALLED` 的中文原因。新的候选内容可重置停滞计数；同内容更换请求键、精确回执重放以及仅增长思考文本不算进展。此策略是连续截断停滞保护，不限制正常 MCP 修正次数，也不提高 OpenCode/Provider 的单次输出额度。取消、既有任务时限与预算继续生效；不另开 Session、不增加 Stage/Task 尝试、不重做 Git 采集。任务删除在同一事务中显式清理续接记录，失败回滚必须保留请求与原批次。
...
新任务在确认时保存 `REPORT_LAYOUT_V3` 的四份固定模板正文及 SHA-256；章节、表格、空数据说明与评分展示由服务端持有，模型不能选择格式。历史 V2 合同继续使用冻结模板，缺少此字段的旧合同使用保留的 V1 渲染器。格式定义见[固定报告格式](template-report-format.md)。服务端以已验证候选生成完整 Markdown，先冻结制品及 SHA-256，再原子写文件；各版报告与评审证据保留。代码审查生成总结、完整问题清单、提交目录与逐提交详情；贡献任务生成项目总结、个人详情、问题清单与评分标准。主子报告复用已验证候选，不增加模型会话。正文按制品 ID 加载，必须校验所属 Task，页面支持安全 Markdown 预览、同版相对链接和单文件/整包下载。V7 的代码审查与贡献周报在完整覆盖、候选引用/归属、冻结公式及报告写入校验通过后记录成功执行轮次；全部会话停止已有证明、执行子状态收束并安全释放队列与租约后自动 `COMPLETED`，不创建 Judge 批次、评审会话或聚合评审输入，手动重试双评审接口拒绝此类任务。完成表示报告流程结束，不表示分析结论经过独立 AI 复核；证据引用与 limitations 保留。

历史 V1–V6 继续按冻结合同进入同批双 Judge，评审对象是报告覆盖、事实、归属和评分依据，发现代码缺陷或低分不构成报告失败。仍须本轮双 Judge PASS 且执行子状态和租约收束后自动完成；不静默升级旧任务或以人工认可替代历史双评审。普通任务的双评审要求不受影响。

详情从服务端读取冻结策略与报告数量；V7 不展示双评审入口、空评审卡或 0/2 指标，报告完成文案为已校验并保存。持久化报告制品、提交阶段验证结果后分别发送元数据失效事件，overview 数量变化也触发一次 audit 元数据刷新，正文仍只在预览时读取。已有报告未选择时显示已生成，加载或读取失败与待生成分别展示，未完成和历史报告仍可预览下载。

### 模板批次收尾与诊断

V111 为模板批次追加独立的收尾意图、停止证明、人工命令及观察记录。历史有效候选回执仍按原冻结合同校验，收尾不升级模板、改写输入或重新调用模型。候选接受不直接完成批次：以回执持久化时间为基准保留 60 秒正常结束窗口，避免 MCP 响应未交付就中断；远端先结束则立即复核结果，超出窗口由任务协调器仅对精确绑定的 Session 请求停止。`action=STOP` 是模型指示，不是停止证明。该窗口是接受后的资源收尾，不是新增的业务生成超时。

收尾意图绑定批次、Session 和请求摘要，网络操作在事务外，返回后短事务复核当前 Task、Attempt、批次代次及版本。正向证明只允许独立终态、abort 明确确认或精确不存在；先持久化证明，再完成批次，因此消息完成时间缺失或后续读取故障不覆盖已保存的证明。未知停止保持运行和占用，停止请求重试至少间隔 15 秒，不新建会话、重发提示或释放租约。取得证明后复用已接受候选，即使受控中止被远端报告为失败，也不把它当成分析失败；用户取消 Task 仍优先，迟到结果不能改写取消状态。

本地 UI 可按版本和幂等命令发起“结束会话并收尾”或“停止此批次”。前者仅对已接受结果，后者仅对未接受结果的当前运行批次；两者不改变其他批次。人工停止意图落库后关闭新候选提交及长度续接，精确回执重放保留。停止确认后未提交结果的批次记为失败，仍按其余独立工作结束后的既有选择重试规则处理。接口不提供原 Session 任意追加消息、密码读取或整任务重新执行。

协调器在后台最多每 15 秒记录一次有界会话观察；检查时间、内容活动时间和候选进展时间分开。内容指纹在展示裁剪前按当前精确请求的消息计算，数据库只保存摘要而不保存正文。重复轮询、幂等重放和换请求键提交相同内容不算新进展；新的候选内容或接受结果才更新候选进展。五分钟未观察到内容变化只标记疑似停滞，不自动判失败或停掉尚未交卷的模型；90 秒无新观察或读取失败提示连接未确认。

`/api/tasks/{taskId}/session-diagnostics` 提供当前批次的需要关注、未完成、全部筛选及稳定游标分页（默认 50、最大 100）。单批详情同时给出接受回执状态、远端观察、停止证明、恢复能力和精确本地/远端标识；不返回提示、候选正文、认证信息或模型源码输出。界面仅在展开诊断后展示标识与目录，复制白名单摘要，恢复写操作必须有本地 UI 标识。摘要读取无远端调用；页面刷新不能制造活动或完成事实。任务删除在既有同一事务中先清理新增记录，回滚保留原证据。

V116 为模板批次追加传输检查记录与独立重试计数。会话状态、精确创建/消息身份查询及输出读取的可恢复传输异常留在当前批次，按 5、10、20、40、60 秒退避并保持最大 60 秒间隔；查询不消耗重新分析次数，不把单次异常升级为整任务失败。记录操作、错误类别、安全分类原因、首次/最近时间、连续次数和下次检查时间，不保存原始异常中的 URL、响应体或凭证。连接/认证/网关等环境阻断期间暂停派发新模型请求，保留既有会话；恢复后继续原身份。未知创建/投递先核对精确身份，不能盲目重复投递。新模型重试规则由[批次失败与手动重试](document-template-contract.md#批次失败与手动重试)持有。

取消任务先持久化 STOPPING，再串行取得当前任务 worker 的执行权。已知 Session 直接核对终态或请求 abort，不强制先读会话列表；状态查询失败不能阻止独立 abort 确认。只有创建身份尚未明确时才需要精确列表核对。abort 必须返回明确确认或精确不存在，未知停止继续保留租约、批次占用与检查退避，不假报取消成功。人工检查可通过 `/api/tasks/{taskId}/session-diagnostics/{batchId}/check` 携带批次版本提前下次检查；它不创建新模型请求或改变重试预算。

历史传输故障导致的 WAITING_INPUT 保留原会话，不后台连带停止其他批次；显式单批停止仍可处理。`/api/template-tasks/{taskId}/recheck` 按 Task 版本恢复原 Attempt、执行周期和累计预算，仅允许可恢复的运行环境等待；取消、预算/时限等待及身份安全冲突不能借此继续。服务端 failed-batches facets 返回恢复能力、阻断数量和 Task 版本，页面不自行推断动作许可。诊断分别展示传输检查与自动重新分析次数，并保留失败清单，停止未知时仍有原因及重新检查入口。
...
模板报告的业务超时遵循[架构合同](architecture.md#业务超时开关)：新任务默认关闭总时长和整个分析尝试的累计时限；主动开启才按冻结时长检查。历史无开关合同继续按原时限恢复。已完成批次、报告验收及停止证明不因关闭超时而改变。

模板版本 8 保持版本 7 的验收与报告策略，增加默认 4、可配置的独立批次窗口；历史版本仍串行恢复。详细并发与冻结规则见[文档模板合同](document-template-contract.md#模板批次并发窗口)。

模板批次失败后先记录并继续剩余独立批次，本轮完成后统一选择重新触发；版本 8 的窗口和版本 7 的报告验收策略保持。已证明终止的失败只影响对应批次，成功结果不重做，其他独立批次继续。新代次拥有独立会话、候选和不可变旧记录；手动重试不重置总预算或停止证明。完整恢复、参数查询与预检规则由[文档模板合同](document-template-contract.md#批次失败与手动重试)持有。
