...
角色页桌面端采用独立滚动的目录与详情，分页固定在目录底部；窄屏恢复上下排列。描述页以流程图标出当前角色负责的节点，保留阶段所用版本入口。MCP 与权限规则分别折叠。Prompt 阅读页合并静态片段为连续正文，清单声明或正文显式占位符中的模板变量统一显示为 `{变量名}` 并标注用途；没有显式变量时不显示变量区域。此阅读投影不改变导出内容、冻结修订或运行时装配，不把不同工作流选用的静态内容视为某次会话的完整输入。
