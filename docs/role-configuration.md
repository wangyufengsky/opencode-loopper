...
设置 → 角色管理（`/settings/roles`）可查看、按版本导出配置 ZIP。导出的清单和 Markdown 文件可在外部编辑，再通过页面校验、检查差异并发布。首版没有在线创建/编辑表单；通过配置包可以新增使用现有适配器的角色。
...
角色页区分配置声明、项目条件下的预估和会话已冻结权限。预估共用适配器上限和权限交集编译，工具策略通过纯读取路径取得，不登记策略、不创建 Session、不调用 Provider。未取得实际 MCP 工具目录或 Task/Stage 授权时明确返回不完整原因，不能把预估当成已获得运行授权。
