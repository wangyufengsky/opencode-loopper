...
辅助服务位于 `/api/assist-mcp-streamable`，随受管 OpenCode 配置注入，独立于公开六工具和内部候选提交服务。传输只接受 literal loopback、当前代际 bearer；每次工具调用还必须持有服务端签发的 `scope`。该凭证绑定外部 Session、代际、项目、Task／Stage／Attempt、Designer 或知识会话身份，模型不能以请求参数替换这些身份。首次创建与精确恢复均在交出 Session 前登记同一份冻结辅助权限；停止、结束、所有者改变或代际变化会使作用域失效。兼容模式不获得辅助凭证。
...
新配置由服务端按类型编译 `driverProfile`、文件与类名，拒绝不匹配的客户端驱动身份。GaussDB 默认 `gaussdb-opengauss-3.1.0`，openGauss 默认 `opengauss-3.1.0`，均使用原始 `opengauss-jdbc-3.1.0.jar`、`org.postgresql.Driver` 与 `jdbc:postgresql:`。历史 `opengauss-7.0.0-RC3-og`（`org.opengauss.Driver`）及 `opengauss-6.0.3`（`org.postgresql.Driver`）保持原 JAR、SHA、类与协议，不重定义 profile。新建及明确编辑测试/保存采用当前默认；仅启停等元数据更新、读取、列表测试及冻结任务恢复不切换驱动。已有 GaussDB 可以明确编辑迁移到受管 3.1.0；GoldenDB 不属于 MCP-database 支持清单，保持历史查看、停用、归档及精确恢复。未带 profile 的历史配置仍从管理员原有 `jdbc-drivers` 按原文件和类加载。
...
服务端生成的二进制产物回执兼容 Windows 分隔符，读取时规范为内部协议路径并继续执行目录、符号链接及哈希校验；用户提交的工作区和输出路径仍按严格协议校验。
