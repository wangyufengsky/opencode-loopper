...
候选 OpenCode Session 必须使用独立的最小权限 profile：Decomposer、工作包设计、滚动计划、Reviewer 和项目公约候选只保留形成仓库证据所需的 `read / glob / grep`，交互式工作包候选可额外使用 `question`，验收闭集选择不开放任何内置工具；每个 profile 都只可见精确命名的私有内部 `submit_candidate`，不可见用户 MCP。该内部 MCP 仅允许 Loopper 受管 OpenCode 通过 loopback 和代际 bearer 调用，不属于公共六工具目录。Router 的单次零工具边界、Requirement/Risk Judge 的既有只读与隔离边界均不改变，也不得借候选 profile 获得内部提交工具。
