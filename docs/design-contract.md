...
文档模板的需求开发采用[独立模板授权](document-template-contract.md#需求开发)，源码单测采用[源码模板授权](source-template-contract.md#单元测试开发)：发起时冻结范围内的设计确认、单包或滚动包 Start 与测试推进许可，不能启用普通 Designer 的推荐业务答案策略。两类来源分别持久化并通过共享授权查询识别，不改变上表普通 Designer 的滚动包人工边界。
...
默认皮肤为“spdb风”，参考浦发银行官网的蓝、红、白视觉，并配有浦发银行相关主页插画；“科技蓝”保留既有深色科技外观，“GitHub 白”参考 GitHub Primer 的浅色视觉。主页右上角提供皮肤选择，立即全站生效，保存在当前浏览器并在刷新、深层链接和同源标签页恢复/同步。未保存、已删除的主题或存储不可用时安全回到 spdb风；历史有效的手动选择继续保留，不改变业务数据或流程。

配色、字体、常用圆角、阴影、渐变装饰、主要按钮状态和首页图片呈现统一由类型化皮肤配置持有。页面使用语义样式变量，不硬编码颜色或判断具体皮肤 ID。新增皮肤只新增配置并注册；布局、滚动和状态语义由组件合同保持。主题作用于 html，覆盖浮层；应用挂载前恢复主题，运行时切换不刷新页面、不清空表单或编辑器内容，代码与流程图同步配色。

配置接口与扩展方式见[皮肤配置与扩展](design/skins.md)。科技蓝基准变量如下。
...
手工保存与确认必须携带编辑器开始编辑时的草稿 `expectedVersion`，服务端以同一版本
快照校验并通过 CAS 提交。后台刷新不得把未保存内容的版本基线推进到新版本；冲突时
保留本地内容、显示冲突并阻断确认，用户明确重新载入后才切换编辑基线。
...
首次附件请求开始后，重试必须复用原草稿、正文、附件和 submission ID；等待期间的新输入
保留为后续未发送内容。响应未知不能通过新建草稿或会话盲目重派。所有任务确认弹窗
同时冻结目标身份和动作版本；路由切换或组件卸载后，不得把旧确认应用到新目标。
操作失败说明与后台读取失败分别保存，成功轮询不能清除尚未处理的操作失败。
...
新建持久会话默认冻结 V2 profile 与唯一 `submit_package_design_v2` 工具，旧 V1 会话不升级，显式关闭只控制新会话。V2 自动准备冻结证据；编译后的 Stage/GIT_DIFF 路径遵守明确文件范围及禁止删除约束，普通单包语义范围不等于技术栈默认路径白名单。候选路径错误允许设计师修正，集合删除和指代文件移动不能经候选分支绕过。兼容、诊断及验证边界见 [V2 加固与启用](package-design-v2-enablement.md)。

提交任务变更的格式为 `#四位数字_提交说明`。AI 建议、标题兜底和用户编辑的说明统一将换行与连续空白合并为空格；预览与实际提交使用同一单行内容。说明最多120个 Unicode 字符，用户输入超限不得静默截断。服务端独立校验工单号、空说明、长度和控制字符，并分别返回具体原因。格式规范化不代替用户的提交、推送或合并确认。

人工点击恢复当前工作包设计不受已经耗尽的自动重设计次数限制；恢复仍检查当前身份、冻结需求、总调用预算和运行环境。自动重试保留既有次数及无进展限制，人工恢复不会重置累计调用账本。

## 设置与需求文档界面

设置页采用分区导航与单面板布局，分为服务、模型、执行限制、重试等待、发布网络和开发辅助。切换分区保留本地未保存编辑，保存沿用完整配置接口，成功后立即回读运行环境并更新左下角的默认模型显示；已有会话仍保持冻结模型。字段校验失败时切换至对应分区显示原因。桌面表单按控件底部对齐，两项字段使用两列，窄屏降列；生效范围按字段和面板明确显示。

模板任务和补充需求共用文档选择入口，显示已选数量、总大小、完整文件名、类型、大小及逐项移除；取消重选保留当前列表，提交期间禁止选择和移除。选择文件不发起上传或启动任务，原有格式与大小校验保持。上传完成后的文档摘要统一呈现文件名、分段数与提取局限，原文和目录仍按需读取。

模板任务目录与右侧参数面板按内容独立确定高度，字段标签紧邻输入，不随目录高度拉伸。参数面板以模板说明、分区表单和底部操作呈现；源码模板分为所属项目、源码与输出、补充要求，并区别测试目录与文档目录。范围预检结果就近展示，创建与正式开始保持分离；桌面使用双栏，窄屏按自然文档流降为单栏，配色沿用当前皮肤。

## 业务时限设置

设置页“全局执行上限”提供默认关闭的“启用业务超时限制”，仅开启时展示任务总时长、尝试和设计时长。验证超时保持独立可见。普通任务合同编辑器无损往返开关并在关闭时显示“未启用超时限制”；旧合同缺失字段展示原启用语义。全局保存提示新任务生效，不声称改写历史运行。数据库连接抽屉使用完整 JDBC URL、用户名及密码；第三方来源工具列表提供完整目录下的一键全部关闭及整批错误提示。

## 项目知识库

知识库采用聊天为中心的默认界面，按需展开来源与引用面板；布局阈值、模型及授权冻结、历史和停止语义统一见 [项目知识库合同](knowledge-contract.md)。它不进入设计与执行流程。
