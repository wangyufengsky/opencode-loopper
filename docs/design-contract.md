...
主页复用 dark-first tokens、现有侧栏和 PageHeader；提供项目、设计与执行规范、任务、待处理中心、历史设计、质量与用量、模板任务、运行环境、工具和设置的普通路由入口。“开始设计”只跳转设计页，不创建任务、确认设计或触发执行。主页不加载全量项目或任务，不显示模拟统计、进度或服务健康结论。
