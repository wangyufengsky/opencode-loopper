...
`GET /api/projects/{id}/story-binding-capability` probes the selected runtime's project-scoped `GET /command`, with no Session/model/statistics side effects. Only the exact registered `aicoding` name enables the creation switch. `POST /session/{id}/command` receives the fixed command, generated start/continue/complete arguments, and a durable `msg_loopper_aicoding_...` message ID. Accounting command POST reads have no automatic deadline; connection establishment remains bounded. The only automatic fallback is one continue after an explicit start failure on the same remote Session and story identity. UNKNOWN, cancellation, and a failed continue are never redispatched. V66 exposes a global live-output dialog and explicit cancellation. Cancellation claims the command barrier, checks the exact latest remote user message before any abort, and releases business work after the stop attempt; it never issues a later abort after releasing business work. Already received reports cannot be retracted. CANCELLED is immutable against late replies. Ordinary HTTP and business execution deadlines remain bounded; BEGIN waits are excluded from their owner business time budgets. See `story-binding.md` for activity and cancellation endpoints.
