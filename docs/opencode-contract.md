...
新增或扩展功能涉及 AI 查询业务数据、提交候选或请求程序动作时，优先使用职责明确、结构化的 MCP 工具，避免把解析最终自然语言或约定文本标记作为默认业务协议。工具应返回校验失败的具体原因和可执行修正指令，让可修正问题在同一会话内反馈、修正、重交；写入保持身份、版本、幂等和权限校验，MCP 调用成功不替代服务端业务验收。

确需其他通道时，在对应功能合同说明原因及反馈方式；历史任务仍遵守冻结协议。通道选择不隐含单次输出额度、会话轮数或任务总预算限制，各轴按下表和相应冻结合同独立定义。本节承接根公约中的既有原则，不改变现有角色的通道例外或恢复行为。

## 配置角色与传输适配

业务角色通过 [角色配置合同](role-configuration.md) 绑定到既有 SessionProfile 或独立统计命令适配器。角色配置只收窄当前授权；候选提交工具、阶段所有权、冻结数据范围、停止证明与下列预算轴继续由服务端决定。旧流程不因绑定发布而重建 Prompt 或权限。
...
The default `JudgeDecisionCandidateWorkflow` must assemble `taskAllPackages(launch.taskId())` before durable initial dispatch, preserving its candidate message ID. Legacy Judge coverage does not prove the candidate path works. MCP Judge prompt assembly removes only the exact server-owned legacy response-format suffix, retaining all frozen evaluation facts; it must not simultaneously request Markdown sections and a single-line reason. Reviewer candidates require `limitations` as a string array, not prose; type errors identify the exact JSON pointer, while unknown authority fields still fail closed.
...
`submissionRevision`. `actual` is rendered from the exact bounded candidate-local value at that pointer; a root
`/candidate` problem uses only the object type, UTF-8 size and top-level-field summary rather than echoing the full
candidate. Every diagnostic text and allowed value is bounded by UTF-8 bytes, not Java character count; any field-level
truncation sets `diagnosticsComplete=false / truncated=true` without converting a retryable rejection into an internal
failure. Semantic problems additionally identify the affected fact key, stage or evidence item and, when required to disambiguate the
...
`GET /api/projects/{id}/story-binding-capability` probes the selected runtime's project-scoped `GET /command`, with no Session/model/statistics side effects. Only the exact registered `aicoding` name enables the creation switch. `POST /session/{id}/command` receives the fixed command, generated start/continue/complete arguments, and a durable `msg_loopper_aicoding_...` message ID. Accounting command POST reads have no automatic deadline; connection establishment remains bounded. The only automatic fallback is one continue after an explicit start failure on the same remote Session and story identity. UNKNOWN, cancellation, and a failed continue are never redispatched. V66 exposes a global live-output dialog and explicit cancellation. Cancellation claims the command barrier, checks the exact latest remote user message before any abort, and releases business work after the stop attempt; it never issues a later abort after releasing business work. Already received reports cannot be retracted. CANCELLED is immutable against late replies. Ordinary HTTP deadlines remain bounded; enabled business deadlines follow the frozen timeout policy; BEGIN waits are excluded from their owner business time budgets. See `story-binding.md` for activity and cancellation endpoints.

Managed config appends the bundled accounting guard while retaining operator plugins, and installs the accounting Agent. Its ordered permission map must emit wildcard deny before aicoding allow: OpenCode uses the last matching rule, so unordered Map.of is invalid. The guard filters statistical turns from business model context (and business context from accounting turns), and checks tool-call parent identity before allowing a statistics round to execute anything except aicoding tools. It does not register the aicoding command. Only Requirement/Package Designers and Implementation are accounted; every new eligible Session uses start. Managed Designer Session policies include an aicoding_* exception because Session deny-all overrides Agent permissions. The guard narrows tools per message, hiding question/business tools during accounting and aicoding tools during Loopper business; execution checks enforce the same boundary without replacing Session permissions. Ordinary manual OpenCode Sessions remain unaffected. BEGIN barriers project RUNNING and no pending business questions until business dispatch. Ordinary role permissions remain authoritative for business work. Business prompts explicitly restore their Agent/model; output/status readers independently exclude accounting user messages and assistant children, including late errors. Command completion is advisory, never a Task/Judge outcome.
...
Ordinary abort and accounting commands share a per-remote gate: cleanup waits for a pending command reply, while explicit statistics cancellation releases only its own message gate, including a late/unresponsive HTTP request. New starts wait for retired Sessions in the same binding chain to settle their completion attempt; success is not required. An explicit start failure appends one fresh continue call/message linked to start by `retry_of`; successful recovery suppresses the start failure notification, while fallback failure emits one notice. V67 manual retries also append a fresh linked identity, preserve the failed operation, and require the remote business owner to retire. The latest failed/unknown/cancelled phase can be retried once per user action; concurrent clicks cannot duplicate dispatch or alter prior evidence.
