...
模板任务暂时关闭故事统计：新建参数表单不展示绑定开关和编号，不探测统计能力，也不提交故事配置；服务端不为新模板 Task 建立故事绑定。旧客户端的新请求若携带 `story.enabled=true`，返回 `TEMPLATE_STORY_ACCOUNTING_DISABLED`，不创建任务；省略配置或显式关闭均可正常创建。已确认请求先按原摘要幂等返回原任务。历史已绑定模板任务继续按冻结绑定恢复和收尾，不删除绑定、会话或调用记录；这些历史任务仅对实际派发的报告分析 ExecutionSession 按 IMPLEMENTATION 统计，程序 Git 采集、Markdown 渲染、缓存命中与双 Judge 不制造实施用量。普通设计与实施任务的统计保持原合同。
...
统计弹窗独立于设计提交响应，因此设计师的首条业务提示尚未发送时也可看到统计状态。后端在统计调用开始、状态变化和关闭结果后，通过全局 `GET /api/story-accounting/events` SSE 通知页面。订阅建立后（含重连）补查一次未关闭调用列表，空闲时不定时查询；SSE 保活仅发送注释，不查询数据库或触发页面刷新。只有当前选中的统计仍在进行时，每 1.2 秒刷新该调用的输出，结束后停止刷新；并行调用可切换查看。页面隐藏时暂停查询，恢复可见时补查一次。查询期间到达的事件合并补查，失败的恢复查询最多延迟重试三次；后续事件、重连或恢复可见仍可重新恢复。统计事件仅在持久化提交后发布，浏览器断线不得改变统计或业务结果。完成回执不会阻塞业务，关闭结果会持久化确认，刷新不重新弹出已关闭的历史结果。输出仅来自该统计消息的 assistant 子回复；不展示 HTTP 原始信封，也不读取业务设计稿。
...
停止设计或取消／暂停任务与取消统计是独立动作。普通会话停止一旦登记，就使该 Session 已经排队、仍在等待统计的业务提示失效；统计返回后不能再把旧提问或实施提示发出去。业务提示实际投递与普通 abort 按 Session 协调，显式恢复产生的新提示仍可执行；取消单次统计不使业务提示失效。

已登记停止导致的旧提示取消由原停止流程完成状态收尾，不记录业务失败或消耗重试预算；尤其暂停等待远端停止时，Task 可能尚为 RUNNING。线程意外中断使用独立错误码，仍按真正的 Session 失败处理。同一任务的取消请求与后台 STOPPING 收尾共享正在进行的停止结果，避免慢统计扩大重复停止和过期 Session 版本回写的窗口。
...
使用正式 JAR、真实本机 OpenCode、原生统计工具与本地确定性模型。每个场景独立创建 Git 项目，任务开始前写入 `.gitignore`；模拟插件在等待时持续写入 `.aicoding/runtime`。每次 start、continue、complete 都在独立接收端实际等待 20 秒。失败降级场景先模拟已有故事并让 start 明确失败，再由 continue 恢复；不会连接真实内网。

```bash
node scripts/aicoding/start-qualification.mjs /绝对路径/opencode-loopper.jar --native-tools --matrix
# 将上一命令打印的目录填入此处；在另一个终端执行
node scripts/aicoding/qualify-matrix.mjs /隔离目录/environment.json --run=regression
```

需要 JDK 21、可执行的 `opencode` 与已安装的插件 API；浏览器场景默认使用本机 Google Chrome，可通过 `CHROME_EXECUTABLE` 指定。每次使用不同的 `--run` 名称保留旧证据，或建立新的隔离实例。可用 `--cases=normal,fallback` 选择子集；多个实例各有独立数据、端口和模型接收端。

23 项覆盖正常开启、start 失败后 continue、设计师和执行者三个操作的分别取消、停止设计、开启和完成统计时取消任务、失败或取消后的统计重试与重复点击、验证失败重试、双评审重试、重做、暂停恢复、交接期间取消、浏览器取消与关闭后刷新、等待期间进程中断恢复，以及范围外既有文件修改确认后进入评审和确认窗口取消任务。检查业务终态、实际回执用时、双 Judge 结果、取消不可被迟到结果覆盖、未知结果不重发，以及评审差异不包含忽略文件。`matrix-<run>/results.json` 和各场景 JSON 保存结果，浏览器场景另存截图；监督进程收到 SIGTERM 后只清理本次隔离进程并保留目录和台账。
