...
模板任务在参数表单中使用相同的能力检测与编号校验，配置直接绑定新 Task。模板没有 Designer 会话；只对实际派发的报告分析 ExecutionSession 按 IMPLEMENTATION 统计。程序 Git 采集、Markdown 渲染、缓存命中与双 Judge 不制造实施用量。新旧统计命令的停止、未知结果和恢复边界一致。
