...
1. 读取当前公约、工作区状态和相关合同，明确本次目标与已有修改；只加载需要的实现和测试。
...
4. 可执行内容稳定后分配新版本，运行一次完整门禁。已通过的检查只有在输入改变或出现新风险时才失效；只重复受影响范围，最终候选仍须完整验证。
5. 核验产物与 diff，记录交付证据，暂存本任务文件并本地提交。发布、部署和运行中服务替换按各自授权处理。
...
正式 `verify.sh` 显式关闭 `backend-dev`，恢复前端构建并执行 `clean verify`。CI 与 Release 的默认 Maven `clean verify` 同样执行完整链路：工具链安装、`npm ci`、项目文档/工具检查、`vue-tsc -b && vite build`、Vitest、Node 工具测试和全部 Java 测试。类型检查已包含在 `build` 中，不再单独重复执行一次。
...
正式门禁后检查 `target/opencode-loopper-<version>.jar` 非空，用 `jar tf` 核验 `BOOT-INF/classes/static/index.html` 与 assets，再计算 SHA-256。记录写入 `docs/deliveries/<version>.md`，包括范围、验证命令/结果、JAR 路径/哈希和未执行项；只回填这些结果不递归打包。提交号放在最终回复，避免为了把提交号写进自身提交而反复提交。

没有发布授权时止于本地提交。收到发版授权后：复查提交包含已核验交付，确认版本未被占用，推送该提交及不可移动的 `v<version>` 标签，等待 Release Actions 完成，回读 JAR/脚本/SHA256SUMS 与远端 digest。远端重建产物与本地 JAR 分别记录，不能假定字节相同。部署、活动 JVM 和浏览器加载另行核验。
