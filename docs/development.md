...
真实模型生成脚本 `scripts/qualify-ppt-generation.mjs` 必须显式设置 `PPT_ALLOW_PROVIDER=1`，默认遇到问题保留状态等待人工处理。隔离测试可额外设置 `PPT_DIALOGUE_ACCEPTANCE=1`，由固定验收用例模拟用户回答与明确确认，记录问题及回复，检查确认前 revision 仍为 0、没有页面；这不是代实际用户确认的产品行为。`PPT_PROJECT_ID` 可指定隔离项目，验证项目资料读取到制作的闭环。成功后继续验证局部修改、无关内容保持与旧制品不变。
