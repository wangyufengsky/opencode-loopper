...
Creation-only story identity is persisted before the first Router. Designer links and task links reference one immutable binding chain; task insertion inherits its Designer chain and Recovery explicitly copies its parent link. Only worked Requirement/Package Designer and Implementation Sessions have a unique ordinal and independent BEGIN/COMPLETE calls (each new Session uses start, never continue; other roles and unowned history are excluded) with durable reserved message IDs. The coordinator records intent in a short transaction, performs native command I/O outside it, and records a bounded result. Each phase is submitted at most once; interrupted PREPARED calls become UNKNOWN on startup. Terminal collection follows persisted owner states, not transient OpenCode IDLE. Statistics failures produce a system message/task event and never drive business state, automatic authorization or retry budgets.
