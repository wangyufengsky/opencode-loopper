...
Creation-only story identity is persisted before the first Router. Designer links and task links reference one immutable binding chain; task insertion inherits its Designer chain and Recovery explicitly copies its parent link. Only worked Requirement/Package Designer and Implementation Sessions have a unique ordinal and independent BEGIN/COMPLETE calls. Each new Session begins with start; an explicit start failure creates one durable continue fallback on the same Session and identity, while UNKNOWN/cancellation never does so. Other roles and unowned history are excluded. The coordinator records each intent in a short transaction, performs native command I/O outside it, and records a bounded result with a fresh message ID; continue links to the failed start through `retry_of`. Interrupted PREPARED calls become UNKNOWN on startup. Terminal collection follows persisted owner states, not transient OpenCode IDLE. A recovered start failure emits no failure notice; a failed fallback emits one system message/task event. Statistics never drive business state, automatic authorization or retry budgets.
