...
第二期文档模板的需求提取、独立原文复核、静态实现评审和独立结论复核使用四个专属候选角色，所有者是文档模型运行，读取权限限于冻结文档或代码。需求开发随后复用软件设计、Implementation 与需求/风险双评审。角色权限、候选结构、来源与完成语义由[文档模板任务合同](document-template-contract.md)持有；普通 Designer 的自动采用推荐答案策略不适用于该模板。
