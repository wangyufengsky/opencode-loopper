...
V72 证据在投递模型前、候选校验及事务之外准备，候选策略只校验数据库冻结的来源哈希。
完整范围与 Codex/生产 OpenCode 证据区分见[工作包 Luna 三批优化说明](package-design-luna-optimization.md)。
...
提取结果也只能进入单一 `ReviewerReportCompilation`：确认无问题时允许空 findings 但摘要仍必填；
...
`LOOPPER_ROLLING_PACKAGE_PLAN_V1_ENABLED=false` 只回滚新建议，常驻 policy/writer/adapter 仍恢复既有运行，
且默认切换不扩大机械纠错、模型权限或 post-dispatch fallback 边界。
...
各角色的数据库、文档与任务证据权限由 [内网辅助 MCP 合同](assist-mcp-contract.md) 和统一工具目录持有。服务端在模型投递前附加必需约束，覆盖自定义 Role Pack、模板分析和重试模板的组合路径。评审读取冻结证据，不能以实时查询改变评审材料。

## 文档需求角色

文档模板版本 2 由程序冻结原文并提供按标题或页码读取的 MCP Resource，需求开发直接复用软件设计、Implementation 与需求/风险双评审，不先生成模型需求清单。静态评审使用 DOCUMENT_CODE_ASSESSMENT_V2 和 DOCUMENT_CODE_REVIEW_V2 两个专属角色；后者独立重读原文并检查遗漏和代码结论。历史版本 1 保留需求提取、原文复核及实现评审的四个候选角色。所有者、会话读取凭据和冻结版本保持隔离。角色权限、候选结构、来源与完成语义由[文档模板任务合同](document-template-contract.md)持有；普通 Designer 的自动采用推荐答案策略不适用于该模板。

## 源码模板角色

源码路径的单元测试开发复用软件设计、Implementation 与双 Judge，源码许可独立于上传文档。详细设计使用 `SOURCE_DETAILED_DESIGN_V1`、`SOURCE_DESIGN_REVIEW_V1` 两个只读角色，通过角色专属 MCP 提交候选。完整读取、覆盖、引用、跨模块复核和结果冻结规则由[源码模板任务合同](source-template-contract.md)持有。

## 独立 PPT Agent

`PPT_AGENT` / `loopper-ppt` 是独立演示文稿角色，使用自己的阶段候选、编辑批次和持久运行表，复用受管会话及私有 MCP 传输。仅开放当前运行的 PPT 制作工具与所选项目的 7 个只读知识查询工具；不获得原生读写、命令或第三方 MCP 通配权限。新作品先进行多轮需求沟通并取得独立的明确确认，随后自动授权包含方向选择、方案制作和文件交付，由服务端校验并续接；旧人工会话仍按冻结权限运行。工具结果和模型结束均不替代制作检查、停止证明或导出事实。完整作用域、恢复和页面协议见 [PPT 合同](ppt-contract.md)。
