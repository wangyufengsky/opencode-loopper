...
`PPT_AGENT` / `loopper-ppt` 是独立演示文稿角色，使用自己的阶段候选、编辑批次和持久运行表，复用受管会话及私有 MCP 传输。仅开放当前运行的 11 个 PPT 工具；不获得原生读写、命令或第三方 MCP 通配权限。工具结果和模型结束均不替代用户方向确认、制作检查或导出事实。完整作用域、恢复和页面协议见 [PPT 合同](ppt-contract.md)。
