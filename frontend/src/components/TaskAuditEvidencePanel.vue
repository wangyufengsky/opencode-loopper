...
import type { Artifact, Attempt, JudgeRun, VerifierResult } from '@/types/domain'
    ...
    <div class="audit-content">
      ...
      <template v-else-if="activeTab === 'diff'">
        ...
        <div v-if="changedPaths.length" class="diff-list" aria-label="变更文件">
          <div v-for="path in changedPaths" :key="path" class="diff-row"><span :class="['diff-state', { added: untrackedPaths.has(path) }]">{{ pathState(path) }}</span><code>{{ path }}</code></div>
        </div>
...
.diff-list { display: grid; gap: 1px; margin-top: 12px; overflow: hidden; border: 1px solid var(--color-border-default); border-radius: 9px; background: var(--color-border-default); }.diff-row { display: grid; grid-template-columns: 48px minmax(0, 1fr); align-items: center; gap: 9px; padding: 8px 11px; background: #0a1120; }.diff-row code { min-width: 0; color: var(--color-text-secondary); font: 10px/1.45 var(--font-code); overflow-wrap: anywhere; }.diff-state { color: #fbbf24; font-size: 9px; font-weight: 700; }.diff-state.added { color: var(--color-success); }
