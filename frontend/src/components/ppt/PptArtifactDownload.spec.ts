...
describe('PPT artifact download', () => {
  ...
  it('keeps the existing artifact available when download fails and retries on request', async () => {
    ...
    const wrapper = mount(PptArtifactDownload, { props: { documentId: 'doc', artifact: { id: 'export', name: '季度汇报.pptx', mediaType: 'application/vnd.openxmlformats-officedocument.presentationml.presentation', url: 'https://untrusted.invalid/file' } } })
