...
.page-title-hover { position: absolute; z-index: 20; top: calc(100% + 10px); left: 0; width: min(540px, calc(100vw - 72px)); padding: 10px 12px; visibility: hidden; border: 1px solid rgb(var(--shade-cyan-01-rgb) / 28%); border-radius: calc(var(--radius-control) + 2px); color: var(--color-text-primary); background: var(--shade-surface-12); box-shadow: var(--appearance-page-header-page-title-hover-box-shadow); font-size: 11px; font-weight: 400; letter-spacing: normal; line-height: 1.55; opacity: 0; overflow-wrap: anywhere; pointer-events: none; transform: translateY(-4px); transition: opacity .12s ease, transform .12s ease, visibility .12s ease; white-space: normal; }
...
.page-title-tooltip:focus-visible { border-radius: calc(var(--radius-control) - 2px); outline: 2px solid var(--color-accent-cyan); outline-offset: 3px; }
