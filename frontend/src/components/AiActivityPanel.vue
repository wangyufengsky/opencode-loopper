...
<style scoped>
.ai-activity { position: relative; display: grid; gap: 14px; min-height: 180px; padding: 17px 18px; overflow: hidden; border: 1px solid rgb(var(--shade-ai-01-rgb) / 30%); border-radius: calc(var(--radius-control) + 6px); background: var(--appearance-ai-activity-panel-ai-activity-background); background-size: 220% 100%; animation: thinking-sheen 3s ease-in-out infinite; }
.activity-heading { display: flex; align-items: center; gap: 14px; min-width: 0; }
.thinking-orbit { display: grid; flex: 0 0 auto; place-items: center; width: 38px; height: 38px; border: 2px solid rgb(var(--shade-ai-01-rgb) / 18%); border-top-color: var(--shade-ai-02); border-right-color: var(--color-accent-cyan); border-radius: 50%; animation: thinking-spin 1s linear infinite; }
.thinking-orbit span { width: 8px; height: 8px; border-radius: 50%; background: var(--appearance-ai-activity-panel-thinking-orbit-span-background); }
.thinking-copy { min-width: 0; margin-right: auto; }.thinking-copy strong { display: flex; align-items: baseline; color: var(--shade-text-01); font-size: 13px; }.thinking-copy small { display: block; margin-top: 5px; color: var(--color-text-secondary); font: 9px/1.45 var(--font-code); }
...
.current-activity { min-width: 0; padding: 12px 14px; border: 1px solid rgb(var(--shade-secondary-01-rgb) / 18%); border-radius: calc(var(--radius-control) + 3px); background: rgb(var(--shade-canvas-01-rgb) / 52%); }.current-activity > header { display: flex; align-items: center; flex-wrap: wrap; gap: 7px; margin-bottom: 9px; color: var(--color-text-secondary); font: 8px/1.2 var(--font-code); }.current-activity > header strong { color: var(--color-text-primary); font-size: 9px; }.current-activity > header i { margin-left: auto; color: var(--color-accent-cyan); font-style: normal; }.current-activity :deep(.markdown-document) { max-height: 260px; overflow: auto; color: var(--color-text-primary); font-size: 12px; line-height: 1.65; }
