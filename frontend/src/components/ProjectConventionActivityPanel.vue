...
import type { ProjectConventionActivity } from '@/types/domain'
...
<template>
  <article class="convention-activity" role="status" aria-live="polite" aria-label="项目公约设计师正在处理">
    <div class="activity-heading">
      <span class="thinking-orbit" aria-hidden="true"><span /></span>
      <div class="thinking-copy">
        <strong>项目公约设计师正在处理<span class="thinking-dots" aria-hidden="true"><i /><i /><i /></span></strong>
        <small>{{ activity?.remoteState ? statusLabel(activity.remoteState) : '连接只读 AI 会话' }}</small>
      </div>
...
<style scoped>
.convention-activity { position: relative; display: grid; gap: 14px; min-height: 180px; padding: 17px 18px; overflow: hidden; border: 1px solid rgb(139 92 246 / 30%); border-radius: 12px; background: linear-gradient(100deg, rgb(139 92 246 / 9%), rgb(34 211 238 / 5%), rgb(139 92 246 / 9%)); background-size: 220% 100%; animation: thinking-sheen 3s ease-in-out infinite; }
...
@media (prefers-reduced-motion: reduce) { .convention-activity, .thinking-orbit, .thinking-dots i { animation: none; } }
</style>
