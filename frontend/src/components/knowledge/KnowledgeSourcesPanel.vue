...
const groups = computed(() => [
  { label: '项目代码', items: props.sources.filter(s => s.kind === 'CODE') },
  { label: '项目文档', items: props.sources.filter(s => ['DOCUMENTS', 'DIRECTORY', 'UPLOAD'].includes(s.kind)) },
  { label: '数据库', items: props.sources.filter(s => s.kind === 'DATABASE') },
])
...
async function search(cursor = '') {
  ...
  const ticket = generation; const page = await api.search(props.project, active.value.id, { conversationId: props.conversationId, query: query.value.trim(), cursor })
...
<template>
  ...
  <p v-if="conversationId" class="knowledge-muted">本对话使用已固定的来源范围</p>
  <form class="knowledge-search" @submit.prevent="run(() => search())"><input v-model="query" aria-label="来源搜索" placeholder="选择来源后搜索关键词" maxlength="200"><button :disabled="!active || busy || !query.trim()">搜索</button></form>
  ...
  <section v-for="group in groups" :key="group.label" class="knowledge-source-group">
    <h3>{{ group.label }}</h3><p v-if="!group.items.length" class="knowledge-muted">尚未绑定</p>
    <div v-for="source in group.items" :key="source.id" class="knowledge-source-row">
      ...
      <button class="knowledge-source-title" :class="{ selected: active?.id === source.id }" :disabled="busy || source.state !== 'READY'" @click="run(() => source.kind === 'DATABASE' ? database(source) : browse(source))"><span>{{ source.name }}</span><small>{{ knowledgeStateLabel(source.state) }}</small></button>
