...
const todos = computed(() => activity.value?.todos ?? [])
...
<template>
  <section class="session-monitor card" :style="outputViewportStyle" aria-labelledby="session-monitor-heading">
    ...
    <div v-else class="monitor-layout">
      ...
      <article :class="['session-console', { 'has-todo-dock': selected?.kind === 'IMPLEMENTATION' && pendingQuestions.length === 0 }]">
        ...
        <SessionRoleSummary v-if="selected" :key="`${taskId}:${selectedKey}`" :task-id="taskId" :session-key="selectedKey" />
        <div v-if="selected?.kind === 'IMPLEMENTATION' && pendingQuestions.length === 0" class="todo-dock">
          <OpenCodeTodoProgress
            :todos="todos"
            :capability="activity?.todoCapability"
            :detail="activity?.todoDetail"
            :truncated="activity?.todoTruncated"
            :expanded="todoExpanded"
            @update:expanded="todoExpanded = $event"
          />
        </div>
        <div ref="stream" class="console-stream" aria-live="polite" @scroll="onStreamScroll">
          ...
          <OpenCodeTodoProgress
            v-if="selected?.kind === 'IMPLEMENTATION' && pendingQuestions.length > 0"
            :todos="todos"
            :capability="activity?.todoCapability"
            :detail="activity?.todoDetail"
            :truncated="activity?.todoTruncated"
...
<style scoped>
.session-monitor { margin-top: 16px; overflow: hidden; border-color: rgb(var(--shade-ai-01-rgb) / 27%); background: var(--appearance-session-monitor-panel-session-monitor-background); box-shadow: var(--appearance-session-monitor-panel-session-monitor-box-shadow); }
...
.live-indicator { display: inline-flex; align-items: center; gap: 7px; flex: 0 0 auto; padding: 6px 9px; border: 1px solid var(--color-border-default); border-radius: 999px; color: var(--color-text-muted); font: 700 10px/1 var(--font-code); }.live-indicator span { width: 7px; height: 7px; border-radius: 50%; background: currentColor; }.live-indicator.active { border-color: rgb(var(--shade-cyan-01-rgb) / 32%); color: var(--color-accent-cyan); }.live-indicator.active span { animation: live-pulse 1.2s ease-in-out infinite; box-shadow: var(--appearance-session-monitor-panel-live-indicator-active-span-box-shadow); }
.monitor-layout { display: grid; grid-template-columns: minmax(275px, .38fr) minmax(0, 1fr); min-height: 550px; }.session-list { max-height: 730px; overflow: auto; padding: 10px; border-right: 1px solid var(--color-border-default); background: rgb(var(--shade-canvas-01-rgb) / 34%); }.session-option { display: grid; width: 100%; gap: 9px; margin: 0 0 8px; padding: 13px; border: 1px solid transparent; border-radius: calc(var(--radius-control) + 3px); color: var(--color-text-secondary); text-align: left; background: transparent; cursor: pointer; }.session-option:hover { border-color: var(--color-border-default); background: var(--color-bg-hover); }.session-option.selected { border-color: rgb(var(--shade-ai-01-rgb) / 40%); color: var(--color-text-primary); background: var(--appearance-session-monitor-panel-session-option-selected-background); box-shadow: var(--appearance-session-monitor-panel-session-option-selected-box-shadow); }.session-option-top { display: flex; align-items: center; justify-content: space-between; gap: 8px; }.session-option strong { min-width: 0; font-size: 11px; line-height: 1.35; }.session-option i { flex: 0 0 auto; padding: 3px 5px; border-radius: calc(var(--radius-control) - 2px); color: var(--color-text-muted); background: rgb(var(--shade-muted-01-rgb) / 12%); font: 700 8px/1 var(--font-code); font-style: normal; }.session-option i.running, .session-option i.creating { color: var(--color-accent-cyan); background: rgb(var(--shade-cyan-01-rgb) / 10%); }.session-purpose { display: flex; align-items: flex-start; gap: 7px; color: var(--shade-secondary-09); font-size: 10px; line-height: 1.55; overflow-wrap: anywhere; }.session-purpose :deep(svg) { flex: 0 0 auto; margin-top: 1px; color: var(--color-accent-ai); }.session-option.selected .session-purpose :deep(svg) { color: var(--color-accent-cyan); }.session-option small { color: var(--color-text-muted); font: 8px/1.35 var(--font-code); }
.session-console { position: relative; display: grid; grid-template-rows: auto auto minmax(0, 1fr); min-width: 0; background: var(--shade-canvas-01); }.session-console.has-todo-dock { grid-template-rows: auto auto auto minmax(0, 1fr); }.console-toolbar { display: flex; align-items: center; justify-content: space-between; gap: 15px; min-height: 48px; padding: 9px 14px; border-bottom: 1px solid var(--color-border-default); color: var(--color-text-muted); font-size: 9px; }.console-toolbar > div { display: flex; align-items: center; gap: 9px; }.console-toolbar strong { color: var(--color-text-primary); font-size: 11px; }.console-toolbar .mono { color: var(--color-accent-cyan); }.transport-dot { display: inline-block; width: 6px; height: 6px; border-radius: 50%; background: var(--color-text-muted); }.transport-dot.live { background: var(--color-success); box-shadow: var(--appearance-session-monitor-panel-transport-dot-live-box-shadow); }
.todo-dock { min-width: 0; padding: 12px 15px 0; border-bottom: 1px solid rgb(var(--shade-border-06-rgb) / 34%); background: var(--shade-canvas-12); }.todo-dock :deep(.todo-panel) { margin-bottom: 12px; }
.console-stream { max-height: var(--model-output-max-height); min-height: var(--model-output-min-height); overflow: auto; padding: 15px; scroll-behavior: smooth; }.activity-part { margin-bottom: 12px; overflow: hidden; border: 1px solid var(--color-border-default); border-radius: calc(var(--radius-control) + 2px); background: rgb(var(--shade-canvas-13-rgb) / 72%); }.activity-part header { display: flex; align-items: center; gap: 8px; padding: 7px 10px; border-bottom: 1px solid var(--color-border-default); color: var(--color-text-muted); font: 9px/1.2 var(--font-code); }.activity-part header span { color: var(--color-accent-cyan); font-weight: 800; letter-spacing: .08em; }.activity-part header strong { color: var(--color-text-secondary); font-weight: 600; }.activity-part header time { margin-left: auto; color: var(--color-text-muted); font: inherit; font-variant-numeric: tabular-nums; }.activity-part header time + i { margin-left: 0; }.activity-part header i { margin-left: auto; font-style: normal; }.activity-part pre { margin: 0; padding: 11px; overflow: visible; color: var(--shade-text-16); font: 11px/1.65 var(--font-code); white-space: pre-wrap; overflow-wrap: anywhere; }.activity-part pre.is-collapsed { max-height: calc(5 * 1.65em + 22px); overflow: hidden; }.part-expand-button { display: inline-flex; align-items: center; gap: 5px; margin: 0 10px 9px; padding: 4px 0; border: 0; color: var(--color-accent-cyan); background: transparent; font: 600 10px/1.4 var(--font-ui); cursor: pointer; }.part-expand-button:hover { color: var(--shade-text-09); }.part-expand-button:focus-visible { border-radius: calc(var(--radius-control) - 2px); outline: 2px solid var(--color-accent-cyan); outline-offset: 3px; }.part-thinking { border-color: rgb(var(--shade-ai-01-rgb) / 28%); background: rgb(var(--shade-ai-01-rgb) / 5%); }.part-thinking header span { color: var(--shade-ai-02); }.part-tool { border-color: rgb(var(--shade-warning-02-rgb) / 22%); }.part-tool header span { color: var(--color-session-warning); }
.question-card { margin: 16px 0 12px; overflow: hidden; border: 1px solid rgb(var(--shade-cyan-01-rgb) / 42%); border-radius: calc(var(--radius-control) + 4px); background: var(--appearance-session-monitor-panel-question-card-background); box-shadow: var(--appearance-session-monitor-panel-question-card-box-shadow); }.question-card-header { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 12px 14px; border-bottom: 1px solid rgb(var(--shade-cyan-01-rgb) / 24%); color: var(--color-accent-cyan); }.question-card-header > div { display: flex; align-items: baseline; gap: 10px; }.question-card-header span { font-size: 12px; font-weight: 800; }.question-card-header strong { color: var(--color-text-muted); font-size: 8px; font-weight: 500; }.question-prompt { padding: 14px; border-bottom: 1px solid var(--color-border-default); }.question-header { margin: 0 0 5px; color: var(--shade-ai-02); font: 800 9px/1.2 var(--font-code); letter-spacing: .08em; text-transform: uppercase; }.question-prompt h3 { margin: 0 0 12px; color: var(--color-text-primary); font-size: 12px; line-height: 1.6; }.question-options { display: grid; gap: 8px; }.question-options :deep(.el-radio), .question-options :deep(.el-checkbox) { width: 100%; height: auto; min-height: 44px; margin: 0; padding: 8px 10px; white-space: normal; }.question-options :deep(.el-radio__label), .question-options :deep(.el-checkbox__label) { width: 100%; white-space: normal; }.question-options span { display: grid; gap: 3px; }.question-options b { color: var(--color-text-primary); font-size: 10px; }.question-options small { color: var(--color-text-muted); font-size: 9px; line-height: 1.4; }.custom-answer { margin-top: 9px; }.question-card footer { display: flex; justify-content: flex-end; gap: 8px; padding: 12px 14px; }
.live-thinking { display: flex; align-items: center; gap: 13px; margin-bottom: 14px; padding: 14px; border: 1px solid rgb(var(--shade-ai-01-rgb) / 28%); border-radius: calc(var(--radius-control) + 3px); background: var(--appearance-session-monitor-panel-live-thinking-background); background-size: 200% 100%; animation: thinking-sheen 3s ease-in-out infinite; }.thinking-orbit { display: grid; place-items: center; width: 32px; height: 32px; border: 2px solid rgb(var(--shade-ai-01-rgb) / 20%); border-top-color: var(--shade-ai-02); border-right-color: var(--color-accent-cyan); border-radius: 50%; animation: thinking-spin .9s linear infinite; }.thinking-orbit > span { width: 7px; height: 7px; border-radius: 50%; background: var(--color-accent-cyan); box-shadow: var(--appearance-session-monitor-panel-thinking-orbit-span-box-shadow); }.live-thinking strong { display: flex; align-items: baseline; color: var(--shade-text-01); font-size: 11px; }.live-thinking p { margin: 4px 0 0; color: var(--color-text-muted); font-size: 9px; }.thinking-dots { display: inline-flex; gap: 3px; margin-left: 5px; }.thinking-dots i { width: 3px; height: 3px; border-radius: 50%; background: var(--color-accent-cyan); animation: thinking-dot 1.1s ease-in-out infinite; }.thinking-dots i:nth-child(2) { animation-delay: .15s; }.thinking-dots i:nth-child(3) { animation-delay: .3s; }
.monitor-empty { display: grid; min-height: 220px; place-content: center; place-items: center; gap: 12px; color: var(--color-text-muted); font-size: 11px; }.monitor-spinner { width: 28px; height: 28px; border: 2px solid var(--color-border-default); border-top-color: var(--color-accent-cyan); border-radius: 50%; animation: thinking-spin .9s linear infinite; }.monitor-warning { display: flex; align-items: flex-start; gap: 8px; margin-bottom: 12px; padding: 10px; border: 1px solid rgb(var(--shade-warning-02-rgb) / 28%); border-radius: calc(var(--radius-control) + 1px); color: var(--color-session-warning); background: rgb(var(--shade-warning-02-rgb) / 6%); font-size: 10px; line-height: 1.5; }.monitor-placeholder { padding: 70px 20px; color: var(--color-text-muted); font-size: 10px; text-align: center; }.follow-button { position: absolute; right: 16px; bottom: 14px; display: inline-flex; align-items: center; gap: 6px; padding: 7px 9px; border: 1px solid rgb(var(--shade-cyan-01-rgb) / 30%); border-radius: 999px; color: var(--color-accent-cyan); background: var(--shade-canvas-13); font: 9px var(--font-code); cursor: pointer; }
...
@media (max-width: 1050px) { .monitor-layout { grid-template-columns: 1fr; }.session-list { display: flex; max-height: none; overflow-x: auto; border-right: 0; border-bottom: 1px solid var(--color-border-default); }.session-option { min-width: 230px; }.console-toolbar { align-items: flex-start; flex-direction: column; }.console-stream { min-height: 440px; } }
