  ...
  <section class="session-monitor card" aria-labelledby="session-monitor-heading">
    ...
    <div v-else class="monitor-layout">
      <nav class="session-list" aria-label="任务会话列表">
        ...
        >
          <span class="session-option-top"><strong>{{ sessionLabel(session) }}</strong><i :class="session.state.toLowerCase()">{{ statusLabel(session.state) }}</i></span>
          <span class="mono">{{ session.externalSessionId ?? session.localSessionId }}</span>
          <small>{{ session.kind === 'JUDGE' ? '只读评审' : '执行会话' }} · {{ session.createdAt }}</small>
        </button>
      </nav>

      <article class="session-console">
        <div class="console-toolbar">
          <div><strong>{{ sessionLabel(selected) }}</strong><span class="mono">{{ statusLabel(activity?.remoteState ?? selected?.state) }}</span></div>
