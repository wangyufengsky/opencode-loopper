...
<style scoped>
.designer-discussion-history { margin: 14px 0; overflow: hidden; border: 1px solid var(--color-border-default); border-radius: calc(var(--radius-control) + 6px); background: rgb(var(--shade-surface-01-rgb) / 54%); }
...
.answered-prompt > p { margin: 0 0 5px; color: var(--shade-ai-02); font: 800 9px/1.2 var(--font-code); letter-spacing: .08em; text-transform: uppercase; }
...
.answered-options li { display: flex; align-items: flex-start; gap: 8px; padding: 8px 10px; border: 1px solid var(--color-border-default); border-radius: calc(var(--radius-control) + 2px); color: var(--color-text-muted); }
.answered-options li.selected { border-color: rgb(var(--shade-cyan-01-rgb) / 42%); background: rgb(var(--shade-cyan-01-rgb) / 8%); color: var(--color-accent-cyan); }
...
.final-answer { display: grid; grid-template-columns: auto 1fr; gap: 10px; margin-top: 9px; padding: 9px 10px; border-radius: calc(var(--radius-control) + 2px); background: rgb(var(--shade-success-01-rgb) / 8%); }
.final-answer span { color: var(--shade-success-03); font: 800 9px/1.5 var(--font-code); }
