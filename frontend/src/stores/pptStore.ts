...
const explain = (value: unknown) =>
  value instanceof Error && /[\u4e00-\u9fff]/.test(value.message)
    ? value.message
    : '暂时无法完成操作，请重新核对状态后重试'
...
export const usePptStore = defineStore('ppt', () => {
  const document = ref<PptDocument | null>(null),
    ...
    plan = ref<PptPlan>(emptyPptPlan())
  const sources = ref<PptSource[]>([]),
    assets = ref<PptSource[]>([]),
    jobs = ref<PptJob[]>([]),
    revisions = ref<PptRevision[]>([])
  const messages = ref<PptMessage[]>([]),
    agent = ref<PptAgentStatus | null>(null),
    capabilities = ref<PptCapabilities | null>(null),
    issues = ref<PptIssue[]>([])
  ...
  const messageCursor = ref<string | null>(null),
    revisionCursor = ref<string | null>(null)
  const generation = ref<PptGeneration | null>(null)
  const generationActive = computed(
    () =>
      !!generation.value && !['COMPLETED', 'STOPPED', 'FAILED'].includes(generation.value.state),
  )
  ...
  const active = computed(
    () =>
      ...
      (!!agent.value && !['IDLE', 'COMPLETED', 'STOPPED', 'FAILED'].includes(agent.value.state)),
  )
  ...
  function persist(id: string, value: Pending | null) {
    try {
      ...
      /* The live request still retains its identity. */
    }
  }
  ...
  function mergeMessages(rows: PptMessage[]) {
  ...
  async function readSnapshot(mode: 'all' | 'activity') {
    ...
    try {
      ...
      if (document.value?.revision !== summary.revision) {
        issues.value = []
        checkedRevision.value = null
      }
      document.value = summary
      deck.value = scene
      plan.value = design.plan || emptyPptPlan()
      sources.value = resources.sources
      assets.value = resources.assets
      jobs.value = runs
      revisions.value = [
        ...new Map(
          [...revisions.value, ...history.items].map((row) => [row.revision, row]),
        ).values(),
      ].sort((a, b) => b.revision - a.revision)
      if (revisions.value.length <= history.items.length)
        revisionCursor.value = history.nextCursor ?? null
      messages.value = mergeMessages([...messages.value, ...chat.items])
      if (messages.value.length <= chat.items.length) messageCursor.value = chat.nextCursor ?? null
      agent.value = status
      generation.value = flow
  ...
  async function load(id: string) {
    ...
    await Promise.all([
      ...
      api
        .capabilities()
        .then((value) => {
          if (ticket === epoch) capabilities.value = value
        })
        .catch((failure) => {
          if (ticket === epoch) error.value = explain(failure)
        }),
    ...
    subscription.onerror = () => {
      if (owner === id) disconnected.value = true
    }
  }
  async function execute(request: Pending) {
    ...
    try {
      ...
      if (request.kind === 'generate') await api.generate(id, revision, String(p.prompt), key)
        ...
        await api.action(id, String(p.action), revision, key, p.extra as Record<string, unknown>)
      else if (request.kind === 'message')
        await api.send(id, {
          expectedRevision: revision,
          idempotencyKey: key,
          text: String(p.text),
          scope: p.scope as PptScope,
        })
      else if (request.kind === 'reply')
        await api.reply(id, String(p.questionId), {
          expectedRevision: revision,
          idempotencyKey: key,
        ...
        await api.createJob(
          id,
          p.kind as 'PREVIEW' | 'EXPORT',
          revision,
          key,
          p.slideId as string | undefined,
        )
      clearAcceptedDraft(id, request)
      persist(id, null)
      if (ticket !== epoch) return false
      pending.value = null
      await refresh()
      return ticket === epoch && !error.value
    } catch (failure) {
      if (ticket === epoch) {
  ...
  function mutate(
    ...
    if (pending.value) {
      error.value = '上一项操作结果尚未核对，请先重试原操作'
      return Promise.resolve(false)
    }
    return execute({
      kind,
      key: crypto.randomUUID(),
      revision: expectedRevision ?? document.value.revision,
  ...
  const savePlan = (value: PptPlan, expectedRevision?: number) =>
    mutate(
      'plan',
      {
        plan: value,
  ...
  const send = (text: string, scope: PptScope) =>
    mutate('message', {
      text,
      scope,
    })
  ...
  async function check() {
    ...
    const id = owner,
      ...
      ticket = epoch
    try {
      const result = await api.checks(id, revision)
      if (ticket === epoch && revision === document.value?.revision) {
  ...
  async function more(kind: 'messages' | 'revisions') {
    const cursor = kind === 'messages' ? messageCursor.value : revisionCursor.value
    if (!owner || !cursor) return
    const ticket = epoch
    try {
      if (kind === 'messages') {
        const page = await api.messages(owner, cursor)
        if (ticket === epoch) {
          messages.value = mergeMessages([...page.items, ...messages.value])
          messageCursor.value = page.nextCursor ?? null
        }
      } else {
        const page = await api.revisions(owner, cursor)
        if (ticket === epoch) {
          revisions.value.push(...page.items)
          revisionCursor.value = page.nextCursor ?? null
        }
      }
    } catch (failure) {
      if (ticket === epoch) error.value = explain(failure)
