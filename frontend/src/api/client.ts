...
function normalizeSettings(value: unknown): AppSettings {
  ...
  return {
    ...
    limits: { templateAnalysisConcurrency: asNumber(limits.templateAnalysisConcurrency, 4), timeoutEnabled: limits.timeoutEnabled === true, maxStageAttempts: asNumber(limits.maxStageAttempts, 3), maxTaskAttempts: asNumber(limits.maxTaskAttempts, 12), sessionErrorLimit: asNumber(limits.sessionErrorLimit, 3), maxDurationMinutes: asNumber(limits.maxDurationMinutes, 120), attemptTimeoutMinutes: asNumber(limits.attemptTimeoutMinutes, 30), verifierTimeoutMinutes: asNumber(limits.verifierTimeoutMinutes, 10), designerTimeoutMinutes: asNumber(limits.designerTimeoutMinutes, 30) },
...
export const api = {
  ...
  discoverProjectGitLab: (id: string) => request<{ repository: string }>(`/projects/${encodeURIComponent(id)}/assist-config/discover`),
  executionEvidence: (task: string, cursor = '') => request<EvidencePage>(`/tasks/${encodeURIComponent(task)}/execution-evidence?${new URLSearchParams({ cursor })}`),
  evidenceFailures: (task: string, cursor = '') => request<EvidenceFailurePage>(`/tasks/${encodeURIComponent(task)}/execution-evidence/failures?${new URLSearchParams({ cursor })}`),
  evidenceFailure: (task: string, id: string) => request<EvidenceFailureDetail>(`/tasks/${encodeURIComponent(task)}/execution-evidence/failure?${new URLSearchParams({ id })}`),
  executionEvidenceBody: (task: string, reference: string, offset = 0) => request<EvidenceBody>(`/tasks/${encodeURIComponent(task)}/execution-evidence/body?${new URLSearchParams({ reference, offset: String(offset) })}`),
  searchExecutionEvidence: (task: string, query: string, cursor = '') => request<EvidenceSearch>(`/tasks/${encodeURIComponent(task)}/execution-evidence/search?${new URLSearchParams({ query, cursor })}`),
  ...
  documentSections: (id: string, fileId: string, offset = 0) => request<DocumentSectionPage>(`/template-tasks/document-runs/${encodeURIComponent(id)}/documents/${encodeURIComponent(fileId)}/sections?offset=${offset}`),
  ...
  recheckTemplateTask: (id: string, expectedVersion: number) => request<{ id: string; state: string }>(`/template-tasks/${encodeURIComponent(id)}/recheck`, { method: 'POST', headers: { 'X-Loopper-Local-UI': '1' }, body: JSON.stringify({ expectedVersion }) }),
  ...
  retrySelectedDocumentBatches: (id: string, batches: TemplateFailedBatch[]) => request<{ id: string; state: string }[]>(`/template-tasks/document-runs/${encodeURIComponent(id)}/batches/retry`, { method: 'POST', headers: { 'X-Loopper-Local-UI': '1' }, body: JSON.stringify({ batches: batches.map(batch => ({ id: batch.id, expectedVersion: batch.version })) }) }),
  ...
  checkTemplateSession: async (taskId: string, batchId: string, expectedVersion: number) => normalizeTemplateDiagnostic(await request<unknown>(`/tasks/${encodeURIComponent(taskId)}/session-diagnostics/${encodeURIComponent(batchId)}/check`, { method: 'POST', headers: { 'X-Loopper-Local-UI': '1' }, body: JSON.stringify({ expectedVersion }) })),
