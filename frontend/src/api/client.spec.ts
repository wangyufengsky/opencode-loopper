...
describe('Loopper REST contract adapter', () => {
  ...
  it('wraps LoopSpec, reads taskId, and maps AVAILABLE to ONLINE', async () => {
    const fetchMock = vi.fn()
      ...
      .mockResolvedValueOnce(json({ loopperVersion: '0.1.46', status: 'OFFLINE', managed: false, endpoint: 'http://127.0.0.1:51234', checkedAt: 'now', startupFailure: 'Managed OpenCode exited with code 1 before it became healthy' }))
    vi.stubGlobal('fetch', fetchMock)
    ...
    await expect(api.getRuntime()).resolves.toMatchObject({
      loopperVersion: '0.1.46', status: 'OFFLINE', managed: false, endpoint: 'http://127.0.0.1:51234',
