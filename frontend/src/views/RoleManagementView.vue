...
const groupedRoles = computed(() => {
  ...
  for (const role of roles.value) {
    const label = role.groupLabel || '专项角色'
  ...
  <main id="main-content" class="content roles-content" tabindex="-1">
    ...
    <div class="roles-layout">
      <section class="card role-list" aria-label="角色列表">
        ...
        <div v-else class="role-items"><section v-for="group in groupedRoles" :key="group.label" class="role-group"><h3>{{ group.label }}</h3><button v-for="role in group.items" :key="role.roleId" type="button" :class="['role-item', { selected: selected?.roleId === role.roleId }]" :aria-pressed="selected?.roleId === role.roleId" @click="selectRole(role.roleId)"><span class="role-heading"><strong>{{ role.displayName }}</strong></span><span class="role-description">{{ role.description }}</span><span class="role-meta">{{ role.activeSlots.length ? `${role.activeSlots.length} 个阶段在使用` : '暂无激活阶段' }} · 最新发布版本 {{ role.latestRevisionNumber }}</span></button></section></div>
      ...
      <section class="card role-detail" aria-label="角色详情">
        ...
        <div v-else-if="!selected" class="empty-detail"><Icon icon="lucide:users-round" width="32" /><h2>选择一个角色查看配置</h2><p>可查看用途、权限估算、提示模板和版本差异。</p></div>
        <template v-else>
          ...
          <div v-if="activeTab === 'overview'" class="detail-section">
            ...
            <ul class="slot-list"><li v-for="binding in activeBindings" :key="binding.slot">
              <strong>{{ binding.label || '角色阶段' }}</strong>
              <span>{{ bindingPurposeLabel(binding) }}</span>
          ...
          <div v-if="activeTab === 'permissions'" class="detail-section"><h3>配置权限与预估</h3><p v-if="revision?.manifest.runtimePolicy === 'ACCOUNTING_COMMAND'" class="page-note">统计辅助使用独立原生命令；工具和授权范围由服务端固定。此处只预览角色的静态指令，不创建会话。</p><p v-else class="page-note">配置声明是权限上界；实际权限还需与项目、会话身份、MCP 连接和服务端策略求交。已运行任务会话的冻结权限摘要请在 <RouterLink to="/tasks">任务详情</RouterLink> 的会话输出中展开查看。</p><p v-if="revisionError" class="error-text" role="alert">{{ revisionError }} <button class="inline-button" @click="loadRevision">重试</button></p><p v-if="revisionLoading" role="status">正在读取配置声明…</p><template v-if="revision"><p>权限模式：{{ permissionModeLabel(revision) }}</p><p v-if="revision.modelPolicy">模型策略：{{ modelPolicyLabel(revision.modelPolicy) }}</p><h4>配置声明的原生工具</h4><p v-if="!revision.nativeTools?.length">无显式原生工具清单。</p><ul v-else class="tool-list"><li v-for="tool in revision.nativeTools" :key="tool">{{ tool }}</li></ul><h4>配置声明的精确 MCP</h4><p v-if="!revision.mcpTools?.length">无显式 MCP 清单。</p><ul v-else class="tool-list"><li v-for="tool in revision.mcpTools" :key="tool">{{ tool }}<small v-if="revision.requiredMcpTools?.includes(tool)">必需</small></li></ul></template><div class="preview-controls"><el-select v-model="selectedSlot" aria-label="选择角色阶段" placeholder="选择阶段"><el-option v-for="binding in availableSlots" :key="binding.slot" :label="binding.label || '角色阶段'" :value="binding.slot" /></el-select><el-select v-model="projectId" aria-label="权限预览所属项目"><el-option label="全局配置" value="" /><el-option v-for="project in projects" :key="project.id" :label="project.name" :value="project.id" /></el-select><el-button :loading="previewLoading" :disabled="!selectedSlot" @click="loadPreview">刷新预览</el-button></div><p v-if="projectsError" class="error-text" role="alert">{{ projectsError }} <button class="inline-button" @click="loadProjects">重试</button></p><p v-if="bindingsError" class="error-text" role="alert">{{ bindingsError }} <button class="inline-button" @click="loadBindings">重试</button></p><p v-if="previewError" class="error-text" role="alert">{{ previewError }} <button class="inline-button" @click="loadPreview">重试</button></p><p v-if="previewLoading" role="status">正在计算配置权限…</p><template v-if="preview"><p class="preview-status">配置预估 · {{ preview.complete ? '信息完整' : '仍需运行时核定' }}</p><ul v-if="preview.limitations.length" class="limitations"><li v-for="(item, index) in preview.limitations" :key="index">{{ item }}</li></ul><h4>权限规则</h4><ul class="permission-list"><li v-for="(rule, index) in preview.rules" :key="index"><span :class="rule.action === 'allow' ? 'allowed' : 'denied'">{{ rule.action === 'allow' ? '允许' : '拒绝' }}</span><strong>{{ rule.permission }}</strong><small>{{ rule.pattern }}</small></li></ul><p v-if="!preview.rules.length">无可展示规则。</p><h4>声明的精确 MCP</h4><ul class="tool-list"><li v-for="(tool, index) in preview.mcpTools" :key="index"><strong>{{ tool.name }}</strong><small>{{ tool.source === 'SYSTEM_REQUIRED' ? '服务端必需 · ' : tool.required ? '角色必需 · ' : '角色声明 · ' }}待会话发现</small></li></ul><p v-if="!preview.mcpTools.length">当前配置没有声明精确 MCP 工具。</p></template></div>
