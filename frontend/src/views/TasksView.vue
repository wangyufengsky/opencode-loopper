...
import { documentTemplateStateLabel, sourceTemplateStateLabel, userFacingError } from '@/utils/displayLabels'
...
const typeFilter = ref<'ALL' | 'TEMPLATE' | 'STANDARD'>('ALL')
...
const visibleTasks = computed(() => {
  ...
  return store.tasks
    ...
    .filter((task) => typeFilter.value === 'ALL' || (typeFilter.value === 'TEMPLATE' ? task.executionMode === 'TEMPLATE_REPORT' : task.executionMode !== 'TEMPLATE_REPORT'))
...
function applyRouteQuery() {
  ...
  typeFilter.value = queryValue(route.query.type) === 'template' ? 'TEMPLATE' : queryValue(route.query.type) === 'standard' ? 'STANDARD' : 'ALL'
...
watch([filter, typeFilter, projectFilter, timeOrder, archiveFilter, search, groupByProject], () => {
  const query: Record<string, string> = {}
  if (filter.value !== 'ALL') query.status = filter.value
  if (typeFilter.value !== 'ALL') query.type = typeFilter.value.toLowerCase()
...
watch([filter, typeFilter, projectFilter, timeOrder, archiveFilter, search], () => {
...
async function reloadTasks(append = false) {
  await store.loadTaskSummaries({
    ...
    ...selectedStatusQuery(),
    taskType: typeFilter.value === 'ALL' ? undefined : typeFilter.value,
...
function destination(task: TaskListItem) { return task.sourceRunId ? `/template-tasks/source-runs/${task.sourceRunId}` : task.documentRunId ? `/template-tasks/document-runs/${task.documentRunId}` : `/tasks/${task.id}` }
function canArchive(task: TaskListItem) {
  return !!(task.documentRunId || task.sourceRunId) ? ['COMPLETED', 'CANCELLED'].includes(task.sourceState ?? task.documentState ?? '')
...
async function toggleArchive(task: TaskListItem) {
  ...
  try {
    if (task.sourceRunId) {
      const current = await api.sourceTemplate(task.sourceRunId)
      await api.sourceTemplateCommand(task.sourceRunId, task.archived ? 'unarchive' : 'archive', {
        requestKey: crypto.randomUUID(), expectedVersion: current.version,
      })
    } else if (task.documentRunId) {
      ...
      <section class="toolbar task-toolbar" aria-label="任务筛选">
        <div class="task-filter-stack">
          ...
          <div class="task-query-row">
            ...
            <el-select v-model="typeFilter" class="project-filter" size="small" aria-label="按任务类型筛选">
              <el-option label="全部类型" value="ALL" /><el-option label="模板任务" value="TEMPLATE" /><el-option label="普通任务" value="STANDARD" />
            </el-select>
...
<style scoped>
.onboarding-card { display: grid; grid-template-columns: auto minmax(0, 1fr) auto; align-items: center; gap: 18px; margin-bottom: 18px; padding: 24px; border-color: rgb(var(--shade-cyan-01-rgb) / 24%); background: var(--appearance-tasks-view-onboarding-card-background); }
.onboarding-icon { display: grid; width: 52px; height: 52px; place-items: center; border: 1px solid rgb(var(--shade-cyan-01-rgb) / 28%); border-radius: calc(var(--radius-control) + 8px); color: var(--color-accent-cyan); background: rgb(var(--shade-cyan-01-rgb) / 8%); }.onboarding-icon svg { width: 24px; height: 24px; }.onboarding-card h2 { margin: 3px 0 7px; font-size: 19px; text-wrap: balance; }.onboarding-card p:last-child { max-width: 680px; margin: 0; color: var(--color-text-secondary); font-size: 12px; line-height: 1.65; }
...
.task-link { display: block; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }.task-branch { margin: 5px 0 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }.numeric { font-variant-numeric: tabular-nums; }.design-history-link { display: inline-flex; align-items: center; gap: 5px; padding: 5px 8px; border: 1px solid var(--color-border-default); border-radius: var(--radius-control); color: var(--color-text-secondary); font-size: 10px; text-decoration: none; }.design-history-link:hover, .design-history-link:focus-visible { border-color: var(--color-accent-cyan); color: var(--color-accent-cyan); outline: none; }.row-actions { display: flex; justify-content: flex-end; gap: 4px; }.icon-action { display: inline-grid; width: 30px; height: 30px; padding: 0; place-items: center; border: 0; border-radius: calc(var(--radius-control) + 1px); background: transparent; color: var(--color-text-secondary); cursor: pointer; text-decoration: none; }.icon-action:hover, .icon-action:focus-visible { background: rgb(var(--shade-cyan-01-rgb) / 9%); color: var(--color-accent-cyan); outline: 2px solid transparent; }.icon-action.danger:hover, .icon-action.danger:focus-visible { background: rgb(var(--shade-danger-01-rgb) / 10%); color: var(--color-task-danger); }.icon-action:focus-visible { outline-color: var(--color-accent-cyan); outline-offset: 1px; }.icon-action.danger:focus-visible { outline-color: var(--color-task-danger); }.icon-action:disabled { cursor: wait; opacity: .55; }.spin { animation: spin .8s linear infinite; }
