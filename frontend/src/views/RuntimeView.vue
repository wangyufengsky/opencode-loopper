...
<template>
  <PageHeader eyebrow="系统" title="OpenCode 运行环境">
    <template #actions><el-button v-if="runtime?.managed" type="primary" :loading="runtime?.status === 'STARTING'" @click="store.restartRuntime"><Icon icon="lucide:rotate-cw" />重启 OpenCode</el-button><el-button v-else-if="runtime?.startupFailure" class="start-runtime-button" type="primary" :loading="startingRuntime" @click="startRuntime"><Icon icon="lucide:play" />启动并检查连接</el-button><el-button v-else type="primary" :loading="runtime?.status === 'STARTING'" @click="store.refreshRuntime"><Icon icon="lucide:rotate-cw" />重新检测</el-button></template>
  </PageHeader>
...
<style scoped>
.runtime-startup-error { margin-bottom: 16px; }.runtime-startup-error .mono { margin-top: 6px; }.runtime-grid { display: grid; grid-template-columns: minmax(340px, .9fr) minmax(380px, 1.1fr); gap: 16px; }.runtime-hero { min-height: 275px; overflow: hidden; background: var(--appearance-runtime-view-runtime-hero-background); }.runtime-orb { display: grid; place-items: center; width: 64px; height: 64px; margin-bottom: 22px; border: 1px solid rgb(var(--shade-cyan-01-rgb) / 42%); border-radius: 50%; color: var(--color-accent-cyan); box-shadow: var(--appearance-runtime-view-runtime-orb-box-shadow); }.runtime-hero h2 { margin: 0 0 13px; font-size: 22px; }.runtime-model { display: flex; flex-direction: column; gap: 5px; margin-top: 23px; }.runtime-model span { color: var(--color-text-muted); font-family: var(--font-code); font-size: 10px; }.runtime-model strong { font-family: var(--font-code); font-size: 12px; font-weight: 500; }.definition-list { margin: 8px 0 0; }.definition-list div { display: flex; justify-content: space-between; gap: 16px; padding: 14px 0; border-bottom: 1px solid var(--color-border-default); }.definition-list dt { color: var(--color-text-secondary); font-size: 12px; }.definition-list dd { margin: 0; color: var(--color-text-primary); font-size: 12px; text-align: right; }
@media (max-width: 900px) { .runtime-grid { grid-template-columns: 1fr; } }
@media (max-width: 640px) { .definition-list div { align-items: flex-start; flex-direction: column; }.definition-list dd { max-width: 100%; overflow-wrap: anywhere; text-align: left; } }
</style>
