...
import type { KnowledgeConversation, KnowledgeSource, KnowledgeContent, KnowledgeCitation, Project } from '@/types/domain'
...
const text = ref(''), pageError = ref(''), newId = ref(crypto.randomUUID())
...
onBeforeUnmount(() => { alive = false; routeSequence++; store.close(); citationSequence++; dataSequence++; if (timer) clearInterval(timer); window.removeEventListener('resize', resize); document.removeEventListener('keydown', keyboard) })
</script>
<template>
    ...
    <div ref="workspace" class="knowledge-workspace" :class="{ 'is-resizing': split.dragging.value }" :style="{ '--evidence-width': `${split.panelWidth.value}px` }">
      ...
      <section class="knowledge-chat" :inert="overlay && (left || right)">
        ...
        <div ref="timeline" class="knowledge-timeline" aria-label="聊天内容">
          ...
          <article v-for="message in displayedMessages" :key="message.id" class="knowledge-turn"><div class="knowledge-user"><span>你</span><p>{{ message.userText }}</p></div><div class="knowledge-answer"><div class="knowledge-answer-label"><Icon icon="lucide:sparkles" /><strong>项目助手</strong><small>{{ knowledgeStateLabel(message.state) }}</small></div><KnowledgeThinking :message="message" :thinking="message.thinkingBody" /><div @click="referenceClick($event, message.citations)"><MarkdownDocument :allow-images="false" :content="markdown(message.answerBody)" /></div><p v-if="message.detail" class="knowledge-muted">{{ message.detail }}</p><button v-if="message.citations.length" class="knowledge-citations-button" @click="openCitation(message.citations[0]!.id, message.citations)"><Icon icon="lucide:files" /> 查看 {{ message.citations.length }} 条来源 · 引用详情 <span>→</span></button></div></article>
        </div>
        <form class="knowledge-composer" @submit.prevent="send"><textarea ref="composer" v-model="text" aria-label="向项目提问" placeholder="关于这个项目，你想了解什么？" maxlength="24000" rows="3" :disabled="store.sending" @keydown.enter.exact="!$event.isComposing && ($event.preventDefault(), send())" /><div class="knowledge-compose-footer"><button type="button" class="knowledge-source-summary" :disabled="!activeProject" @click="openPanel('left')"><Icon icon="lucide:layers" /> {{ store.conversation?.sources.length ?? selected.length }} 项来源</button><div class="knowledge-model"><KnowledgeModelPicker v-if="!store.conversation" :model="model" :default-model="defaultModel" :models="models" :loading="catalogLoading" :error="catalogError" :disabled="store.sending" @load="modelOptions.loadCatalog()" @retry="modelOptions.loadCatalog(true)" @select="modelOptions.select" /><span v-else :title="store.conversation.model">{{ store.conversation.model }}</span></div><button v-if="store.active" type="button" class="knowledge-send" :disabled="store.sending || store.conversation?.state === 'STOPPING'" @click="store.stop">{{ store.conversation?.state === 'STOPPING' ? '确认停止中' : '停止生成' }}</button><button v-else class="knowledge-send" :disabled="!canSend">发送 <Icon icon="lucide:arrow-up" /></button></div></form>
