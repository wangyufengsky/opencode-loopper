...
let citationSequence = 0, dataSequence = 0, timer: ReturnType<typeof setInterval> | undefined
...
const canSend = computed(() => text.value.trim() && activeProject.value && (store.conversation || (selected.value.length && model.value)) && !store.sending && !store.active && mode.value === 'managed')
...
onMounted(async () => {
  ...
  try { const results = await Promise.allSettled([api.getProjects(), api.getSettings(), api.getSettingsModels()])
    const [p, s, m] = results; if (p.status === 'fulfilled') { projects.value = p.value; project.value = p.value[0]?.id || '' } else pageError.value = notice(p.reason)
    if (s.status === 'fulfilled') { model.value = s.value.openCode.model; mode.value = s.value.openCode.mode } else pageError.value = notice(s.reason)
    if (m.status === 'fulfilled') models.value = m.value; else pageError.value = notice(m.reason)
    await loadRoute()
  } finally { initialLoading.value = false }
  timer = setInterval(() => { if (store.active || store.disconnected) void store.refresh() }, 5000)
})
onBeforeUnmount(() => { store.close(); citationSequence++; dataSequence++; if (timer) clearInterval(timer); window.removeEventListener('resize', resize); document.removeEventListener('keydown', keyboard) })
</script>
<template>
  <main id="main-content" class="knowledge-page" aria-label="知识库">
    ...
    <div class="knowledge-workspace">
      ...
      <section class="knowledge-chat" :inert="overlay && (left || right)">
        <div v-if="pageError || store.error" class="knowledge-notice" role="alert">{{ pageError || store.error }} <button @click="store.conversation ? store.refresh() : loadRoute()">重新读取</button></div>
