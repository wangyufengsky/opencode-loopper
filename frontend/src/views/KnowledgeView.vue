    ...
    <div ref="workspace" class="knowledge-workspace" :class="{ 'is-resizing': split.dragging.value }" :style="{ '--evidence-width': `${split.panelWidth.value}px` }">
      ...
      <section class="knowledge-chat" :inert="overlay && (left || right)">
        ...
        <div ref="timeline" class="knowledge-timeline" aria-label="聊天内容">
          ...
          <article v-for="message in displayedMessages" :key="message.id" class="knowledge-turn"><div class="knowledge-user" aria-label="你的消息"><p>{{ message.userText }}</p><img class="knowledge-user-avatar" :src="userAvatar" alt="你的头像" width="36" height="36" /></div><div class="knowledge-answer"><div class="knowledge-answer-label"><Icon icon="lucide:sparkles" /><strong>项目助手</strong><small>{{ message.questions?.some(q => q.state === 'PENDING') ? '等待回答' : knowledgeStateLabel(message.state) }}</small></div><KnowledgeThinking :message="message" :thinking="message.thinkingBody" :answer="message.answerBody" /><div @click="referenceClick($event, message.citations)"><MarkdownDocument :allow-images="false" :content="message.answerBody" :resolve-link="knowledgeLink" /></div><KnowledgeQuestion v-for="question in message.questions" :key="question.id" :question="question" :conversation="store.conversation!.id" @answered="store.refresh" /><p v-if="message.detail" class="knowledge-muted">{{ message.detail }}</p><button v-if="message.state === 'FAILED' && message.id === store.messages.at(-1)?.id && !store.active" :disabled="store.sending" @click="text = message.userText; composer?.focus({ preventScroll: true })">重新提问</button><button v-if="message.citations.length" class="knowledge-citations-button" @click="openCitation(message.citations[0]!.id, message.citations)"><Icon icon="lucide:files" /> 查看 {{ message.citations.length }} 条来源 · 引用详情 <span>→</span></button></div></article>
        </div>
