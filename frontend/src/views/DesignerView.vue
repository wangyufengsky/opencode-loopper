...
async function startDraft() {
  ...
  busy.value = true
  try {
    designerSession.value = await api.createDesignerSession(project.id, goal)
    messages.value = designerSession.value.messages
    draft.value = await api.createDraft(blankSpec(project.id, goal))
    editorValue.value = JSON.stringify(draft.value.spec, null, 2)
...
.designer-layout { display: grid; grid-template-columns: minmax(390px, .9fr) minmax(550px, 1.1fr); gap: 18px; }
.designer-chat, .spec-panel { min-height: 820px; }
.designer-chat { display: flex; flex-direction: column; }
.chat-history { flex: 1; min-height: 260px; padding: 0 20px 20px; overflow: auto; }
.chat-message { margin: 14px 0; padding: 12px; border: 1px solid var(--color-border-default); border-radius: 10px; background: rgb(7 11 20 / 45%); }
.chat-user { margin-left: 30px; border-color: rgb(59 130 246 / 30%); background: rgb(59 130 246 / 8%); }
.chat-assistant { margin-right: 18px; }
.chat-role { color: var(--color-accent-ai); font-family: var(--font-code); font-size: 10px; font-weight: 700; }
.chat-user .chat-role { color: var(--color-accent-cyan); }
.chat-message p { margin: 7px 0; color: var(--color-text-primary); font-size: 12px; line-height: 1.6; }
