...
describe('AutomationsView expected public API contract', () => {
  it('renders loading before the server responds', () => {
    ...
    expect(wrapper.text()).toContain('正在读取自动化记录')
  })
  ...
  it('reads the create mutation wrapper and only offers manual triggering for MANUAL rules', async () => {
    ...
    await wrapper.findAll('button').find(button => button.text().includes('创建规则'))!.trigger('click')
    await flushPromises()
  ...
  it('explicitly updates rules, gates AUTO_START by version approval, and confirms review runs', async () => {
    const wrapper = mountView()
    await flushPromises()
    expect(wrapper.findAll('button').filter(button => button.text().includes('启用并自动开始'))).toHaveLength(1)
    await wrapper.findAll('button').find(button => button.text().includes('启用并人工确认'))!.trigger('click')
    await flushPromises()
    ...
    await wrapper.findAll('button').find(button => button.text().includes('启用并自动开始'))!.trigger('click')
    await flushPromises()
    ...
    expect(wrapper.find('tbody').text()).not.toContain('task-1')
  ...
  it('previews before import, reveals imported webhook tokens once, and exports persisted configuration', async () => {
    ...
    await wrapper.find('textarea[aria-label="导入配置"]').setValue('{"formatVersion":1,"templates":[],"rules":[]}')
    await wrapper.findAll('button').find(button => button.text().includes('预览导入'))!.trigger('click')
    await flushPromises()
    expect(wrapper.text()).toContain('1 个模板 · 2 条规则')
    ...
    await flushPromises()
    expect((wrapper.find('textarea[aria-label="导出配置"]').element as HTMLTextAreaElement).value).toBe('{"templates":[]}')
  })
})
