...
<template>
  ...
  <main id="main-content" class="content" tabindex="-1">
    ...
    <template v-else>
      ...
      <section v-else class="insight-list" aria-label="任务洞察"><article v-for="task in tasks" :key="task.taskId" class="card insight-row"><header><div><p class="eyebrow">{{ task.state }}</p><h2>{{ task.title }}</h2></div><span :class="['quality', `quality-${task.quality.state.toLowerCase()}`]">{{ task.quality.state }}</span></header><dl><div><dt>Token</dt><dd>{{ tokens(task.usage.totalTokens) }}<small v-if="task.usage.unknownUsageCount"> + {{ task.usage.unknownUsageCount }} 未知</small></dd></div><div><dt>耗时</dt><dd>{{ duration(task.durationMs) }}</dd></div><div><dt>重试</dt><dd>{{ task.retryCount }}</dd></div><div><dt>质量</dt><dd>{{ task.quality.deterministicPassed ? '验收通过' : '验收待定' }} · {{ task.quality.requirementJudgePassed && task.quality.riskJudgePassed ? '双评审通过' : '评审待定' }}</dd></div></dl></article></section>
    </template>
  </main>
</template>

<style scoped>
.insight-metrics{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:14px;margin-bottom:16px}.insight-metrics article,.insight-row{padding:16px;background:linear-gradient(145deg,rgb(18 28 48 / 96%),rgb(8 14 28 / 92%))}.insight-metrics span,.insight-metrics small,dt{color:var(--color-text-muted);font:700 10px/1.4 var(--font-code)}.insight-metrics strong{display:block;margin:10px 0 5px;color:var(--color-text-primary);font-size:24px}.insight-metrics .costs{font-size:13px;line-height:1.55}.insight-list{display:grid;gap:12px}.insight-row header{display:flex;justify-content:space-between;gap:12px}.insight-row h2{margin:0;font-size:15px}.quality{padding:5px 8px;border:1px solid;border-radius:999px;font:700 9px/1 var(--font-code)}.quality-pass{color:var(--color-success)}.quality-pending,.quality-review_required{color:var(--color-session-warning)}.insight-row dl{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:1px;margin:15px 0 0;background:var(--color-border-default)}.insight-row dl div{padding:10px;background:rgb(2 6 23 / 40%)}dd{margin:6px 0 0;color:var(--color-text-secondary);font-size:11px}dd small{color:var(--color-session-warning)}.spin{animation:spin 1s linear infinite}@keyframes spin{to{transform:rotate(360deg)}}@media(max-width:800px){.insight-metrics,.insight-row dl{grid-template-columns:1fr}}
</style>
