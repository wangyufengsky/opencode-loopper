...
<template>
  <PageHeader eyebrow="系统" title="详细配置">
  ...
  <main id="main-content" class="content" tabindex="-1" v-loading="loading">
    <section class="settings-layout">
      <article class="card card-pad">
        <div class="card-header"><div><p class="eyebrow">运行环境</p><h2 class="card-title">服务设置</h2></div><span class="activation restart">重启生效</span></div>
        <el-form label-position="top">
          ...
          <div class="form-grid"><el-form-item label="任务监控间隔（秒）"><el-input-number v-model="settings.runtime.monitorDelaySeconds" :min="1" :max="60" /></el-form-item><el-form-item label="设计监控间隔（毫秒）"><el-input-number v-model="settings.runtime.designerMonitorDelayMillis" :min="250" :max="10000" /></el-form-item><el-form-item label="终止清理次数（立即生效）"><el-input-number v-model="settings.runtime.abortCleanupAttempts" :min="1" :max="10" /></el-form-item></div>
        </el-form>
      </article>
      ...
      <article class="card card-pad">
        <div class="card-header"><div><p class="eyebrow">执行限制</p><h2 class="card-title">全局执行上限</h2></div><span class="activation live">立即生效</span></div>
        <el-form label-position="top"><div class="form-grid limits-grid">
          ...
          <el-form-item label="任务总时长（分钟）"><el-input-number v-model="settings.limits.maxDurationMinutes" :min="1" :max="1440" /></el-form-item><el-form-item label="尝试超时（分钟）"><el-input-number v-model="settings.limits.attemptTimeoutMinutes" :min="1" :max="120" /></el-form-item><el-form-item label="验证超时（分钟）"><el-input-number v-model="settings.limits.verifierTimeoutMinutes" :min="1" :max="120" /></el-form-item><el-form-item label="设计超时（分钟）"><el-input-number v-model="settings.limits.designerTimeoutMinutes" :min="1" :max="120" /></el-form-item>
        </div></el-form>
      </article>

      <article class="card card-pad retry-settings">
        <div class="card-header"><div><p class="eyebrow">重试等待</p><h2 class="card-title">分类退避</h2></div><span class="activation live">新计划生效</span></div>
      ...
      <article class="card card-pad publication-settings">
        <div class="card-header"><div><p class="eyebrow">发布</p><h2 class="card-title">发布网络</h2></div><span class="activation restart">重启生效</span></div>
        <el-form label-position="top">
    ...
    <section class="card card-pad settings-demo"><div><p class="eyebrow">开发辅助</p><h2 class="card-title">演示数据模式</h2></div><el-button :type="store.usingDemo ? 'warning' : 'primary'" plain :loading="switchingDemo" @click="toggleDemo">{{ store.usingDemo ? '退出演示数据' : '启用演示数据' }}</el-button></section>
  </main>
</template>
