...
<template>
  ...
  <main id="main-content" class="content" tabindex="-1">
    ...
    <template v-else>
      ...
      <section v-if="!isTemplateTask && task.stages?.length" class="card card-pad" style="margin-top: 16px"><div class="card-header"><div><h2 class="card-title">阶段进度</h2></div></div><StageRail :stages="task.stages" /></section>
...
.result-summary { display: grid; grid-template-columns: minmax(0, 1.2fr) minmax(360px, .8fr); align-items: center; gap: 22px; margin-top: 16px; border-color: rgb(var(--shade-cyan-01-rgb) / 19%); background: var(--appearance-task-detail-view-result-summary-background); }.result-copy p:last-child { max-width: 720px; margin: 8px 0 0; color: var(--color-text-secondary); font-size: 12px; line-height: 1.65; }.result-metrics { display: grid; grid-template-columns: repeat(auto-fit, minmax(100px, 1fr)); gap: 8px; margin: 0; }.result-metrics div { padding: 12px; border: 1px solid var(--color-border-default); border-radius: calc(var(--radius-control) + 3px); background: rgb(var(--shade-canvas-02-rgb) / 50%); }.result-metrics dt { color: var(--color-text-tertiary); font-size: 9px; }.result-metrics dd { margin: 6px 0 0; color: var(--color-text-primary); font: 700 14px/1 var(--font-code); font-variant-numeric: tabular-nums; }
.ai-output-notice { display: flex; align-items: center; gap: 8px; margin-top: 10px; padding: 9px 12px; border: 1px solid rgb(var(--shade-cyan-01-rgb) / 28%); border-radius: calc(var(--radius-control) + 2px); color: var(--shade-cyan-05); background: rgb(var(--shade-cyan-04-rgb) / 8%); font-size: 11px; }
.retry-wait-card { display: flex; align-items: center; justify-content: space-between; gap: 18px; margin-top: 16px; border-color: rgb(var(--shade-warning-02-rgb) / 36%); background: var(--appearance-task-detail-view-retry-wait-card-background); }.retry-wait-card p:last-child { margin: 8px 0 0; color: var(--color-text-secondary); font-size: 11px; }.retry-countdown { color: var(--color-session-warning); font-size: 24px; font-variant-numeric: tabular-nums; }
.decision-successor { display: flex; align-items: center; justify-content: space-between; gap: 16px; margin-top: 16px; border-color: rgb(var(--shade-cyan-01-rgb) / 28%); }
.queue-blocker { margin-top: 16px; border-color: rgb(var(--shade-warning-02-rgb) / 30%); background: var(--appearance-task-detail-view-queue-blocker-background); }.queue-holder { display: grid; grid-template-columns: minmax(0, 1fr) auto; gap: 20px; margin-top: 12px; }.queue-holder > div:first-child { display: grid; align-content: start; gap: 7px; min-width: 0; }.queue-holder-link { width: fit-content; max-width: 100%; padding: 0; overflow: hidden; border: 0; background: transparent; color: var(--color-accent); font: inherit; font-weight: 700; text-align: left; text-overflow: ellipsis; white-space: nowrap; cursor: pointer; }.queue-holder dl { display: grid; grid-template-columns: repeat(3, minmax(82px, 1fr)); gap: 8px; margin: 0; }.queue-holder dl div { padding: 9px 11px; border: 1px solid var(--color-border-default); border-radius: calc(var(--radius-control) + 2px); background: rgb(var(--shade-canvas-07-rgb) / 35%); }.queue-holder dt { color: var(--color-text-tertiary); font-size: 9px; }.queue-holder dd { margin: 5px 0 0; color: var(--color-text-primary); font: 650 10px/1.2 var(--font-code); }.queue-copy { margin: 12px 0; color: var(--color-text-secondary); font-size: 12px; line-height: 1.6; }.queue-error { margin: 12px 0 0; color: var(--color-danger); font-size: 12px; }
...
.package-progress { margin-top: 16px; border-color: rgb(var(--shade-primary-01-rgb) / 22%); }
.package-progress-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(170px, 1fr)); gap: 10px; margin-top: 12px; }
.package-progress-card { padding: 12px; border: 1px solid var(--color-border-default); border-radius: calc(var(--radius-control) + 4px); background: rgb(var(--shade-canvas-07-rgb) / 30%); }
...
.package-progress-card header > strong { color: var(--shade-link-02); font: 750 10px/1 var(--font-code); }
...
.package-progress-card.is-running { border-color: rgb(var(--shade-cyan-01-rgb) / 38%); }
.package-progress-card.is-succeeded { border-color: rgb(var(--shade-success-01-rgb) / 32%); }
.package-progress-card.is-failed { border-color: rgb(var(--shade-border-01-rgb) / 38%); }
