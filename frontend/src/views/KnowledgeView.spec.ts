...
const fetchMock = vi.fn(async (input: string | URL | Request, init?: RequestInit) => {
  ...
  let body: unknown
  if (path === '/api/settings') body = { openCode: { mode: 'managed', ...configured } }
    ...
    if (init?.method === 'POST') {
      ...
      if (!catalog.some(item => item.id === request.model)) return new Response(JSON.stringify({ detail: '所选模型不可用，请刷新模型列表' }), { status: 400 })
...
describe('知识库真实设置接口与模型选择', () => {
  beforeEach(() => {
    ...
    failModels = false; created = []; vi.clearAllMocks(); sessionStorage.clear()
  ...
  it('combines the real separate provider/model fields and sends the exact catalog id', async () => {
    await render(); expect(modelSelect().element.value).toBe('deepseek/shared-model')
  ...
  it('refreshes configuration and catalog without clearing the question or creating a conversation', async () => {
    configured.model = 'retired'; await render(); await question().setValue('保留这个问题')
  ...
  it('keeps an explicitly selected provider when reloading the system default', async () => {
    await render(); await modelSelect().setValue('opencode-go/shared-model'); await question().setValue('继续')
  ...
  it('does not guess a provider from a bare name shared by several providers', async () => {
    configured.provider = ''; await render(); await question().setValue('问题')
  ...
  it('recovers a failed model list request while keeping the draft', async () => {
    failModels = true; await render(); await question().setValue('未发送的内容')
    expect(sendButton().element.disabled).toBe(true); expect(wrapper!.get('[role="alert"]').text()).toContain('模型列表')
    failModels = false; await retry()
  ...
  it('keeps the frozen model of an existing conversation when settings are reloaded', async () => {
    await render('/knowledge/saved'); await question().setValue('追问')
