...
describe('RuntimeView managed startup diagnostics', () => {
  it('shows the launch failure and labels the random port as an attempted address', () => {
    ...
    store.runtime = {
      loopperVersion: '0.1.48', status: 'OFFLINE', managed: false, endpoint: 'http://127.0.0.1:51234', model: '', checkedAt: '2026-08-12T06:00:00Z',
    ...
    expect(wrapper.get('.loopper-version').text()).toContain('0.1.48')
  ...
  it('starts OpenCode explicitly and reports success only after the checked snapshot is online', async () => {
    ...
    const started = {
      loopperVersion: '0.1.48', status: 'ONLINE' as const, managed: true, pid: 6400, endpoint: 'http://127.0.0.1:34020',
