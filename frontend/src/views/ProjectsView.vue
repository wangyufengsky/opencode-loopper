  ...
  <main id="main-content" class="content" tabindex="-1">
    ...
    <section v-else-if="store.projects.length" class="project-grid">
      <article v-for="project in store.projects" :key="project.id" :id="`project-${project.id}`" class="card card-pad project-card" :class="{ 'knowledge-project-target': route.query.project === project.id }">
        ...
        <p v-if="project.repositoryRoot && project.repositoryRoot !== project.rootPath" class="mono tiny muted project-path">Git 仓库：{{ project.repositoryRoot }}</p>
        ...
        <div class="project-footer">
          ...
          <div class="project-actions">
            ...
            <button type="button" class="convention-action" @click="assistProject = project"><Icon icon="lucide:plug" /><span>Git 与 GitLab</span></button>
...
.convention-action { display: inline-flex; flex: 0 0 auto; align-items: center; justify-content: center; gap: 6px; min-height: 30px; padding: 0 10px; border: 1px solid rgb(var(--shade-ai-01-rgb) / 34%); border-radius: calc(var(--radius-control) + 1px); color: var(--shade-secondary-06); background: rgb(var(--shade-ai-01-rgb) / 8%); font-size: 10px; font-weight: 680; line-height: 1; cursor: pointer; transition: color .16s ease, background-color .16s ease, border-color .16s ease, box-shadow .16s ease, transform .08s ease; touch-action: manipulation; }.convention-action svg { width: 13px; height: 13px; }.convention-action:hover:not(:disabled) { border-color: rgb(var(--shade-ai-01-rgb) / 62%); color: var(--shade-text-07); background: rgb(var(--shade-ai-01-rgb) / 17%); box-shadow: var(--appearance-projects-view-convention-action-hover-not-disabled-box-shadow); }.convention-action:active:not(:disabled) { transform: translateY(1px); }.convention-action:focus-visible { outline: 2px solid var(--color-accent-cyan); outline-offset: 3px; }.convention-action:disabled { opacity: .5; cursor: wait; }
.resume-design-action { border-color: rgb(var(--shade-cyan-01-rgb) / 36%); color: var(--shade-cyan-02); background: rgb(var(--shade-cyan-04-rgb) / 10%); }
.danger-action { border-color: rgb(var(--shade-danger-05-rgb) / 28%); color: var(--shade-danger-02); background: rgb(var(--shade-elevated-12-rgb) / 8%); }.danger-action:hover:not(:disabled) { border-color: rgb(var(--shade-danger-05-rgb) / 55%); color: var(--shade-text-05); background: rgb(var(--shade-elevated-12-rgb) / 20%); box-shadow: var(--appearance-projects-view-danger-action-hover-not-disabled-box-shadow); }
.convention-notice { display: flex; align-items: flex-start; gap: 8px; margin-top: 12px; padding: 10px 12px; border: 1px solid rgb(var(--shade-cyan-01-rgb) / 28%); border-radius: calc(var(--radius-control) + 2px); color: var(--shade-cyan-05); background: rgb(var(--shade-cyan-04-rgb) / 10%); font-size: 12px; }
.convention-progress { display: flex; align-items: center; justify-content: center; gap: 10px; min-height: 180px; }.convention-progress p { margin: 4px 0 0; }.convention-empty { display: grid; place-items: center; gap: 9px; min-height: 210px; margin-top: 16px; border: 1px dashed var(--color-border); border-radius: calc(var(--radius-control) + 4px); color: var(--color-text-secondary); background: rgb(var(--shade-surface-01-rgb) / 30%); text-align: center; }.convention-empty svg { color: var(--color-accent-ai); }.convention-empty p { margin: 0; color: var(--color-text-muted); font-size: 12px; }.convention-error { margin: 16px 0; }.convention-meta { display: flex; justify-content: space-between; gap: 12px; margin: 18px 0 9px; }.convention-preview :deep(textarea) { line-height: 1.55; }.spin { animation: spin 1s linear infinite; }
