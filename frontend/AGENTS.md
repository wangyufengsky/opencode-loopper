...
- 遵循 `docs/design-contract.md` 的可配置皮肤、错误层级和桌面优先结构；配色、字体、常用圆角、阴影与装饰在 `themes/` 配置，页面复用语义变量与 `styles/tokens.css`。禁止新增页面硬编码颜色或具体皮肤 ID 判断；新增皮肤只新增配置并注册，扩展方法见 [皮肤配置](../docs/design/skins.md)。
