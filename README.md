...
> 当前版本：`0.4.14`。默认访问 **http://127.0.0.1:8080/**，打开即进入主页。面向单机本地使用，不是多租户远程执行平台。
...
| 文件 | 内容 |
| --- | --- |
| `opencode-loopper-0.4.14.jar` | 后端、前端页面与 SQLite JDBC |
...
```powershell
Get-FileHash .\opencode-loopper-0.4.14.jar -Algorithm SHA256
...
```bash
java -jar opencode-loopper-0.4.14.jar
```
...
cd opencode-loopper
./mvnw clean verify
java -jar target/opencode-loopper-0.4.14.jar
```
