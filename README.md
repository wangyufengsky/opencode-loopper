...
> 当前版本：`0.2.60`。Loopper 默认只监听 `127.0.0.1`，面向单机本地使用，不是多租户远程执行平台。
...
- **Role Pack v6、服务端快速路径与按需 Compiler**：技术标签先归并为 Java、Python、Node 和 Other 软件族，JUnit/Jupiter/Surefire 等测试标签不会把 Java 任务误判为混合栈，工作包识别使用技术词边界，`ChainNodeInvoker` 等业务符号不会误触发 Node。Designer 继续只写受控 Markdown，并用“阶段 / 目标 / 包含场景/评审/交付 / 前置阶段”精确引用前文标题；服务端只做 NFKC、空白和大小写规范化，不删除标点或模糊匹配。完整普通 `WP-1` 直接按 Designer 的 1–6 个阶段完成能力求解、EARS 条件与 LoopSpec v2 lowering，不创建 Compiler Session 或消耗模型调用。只有闭集事实/能力仍有歧义时才调用一次无工具 Compiler 填洞，大型任务也只调用一次并锁定阶段拓扑；非法输出定点退回完整设计，不再丢弃为空建议或降级成通用单阶段。服务端只从正向交付关系发现测试目标，用有界精确集合覆盖唯一派生 `COMPILED` 或具体 `DESIGN_INCOMPLETE`，并生成命令、路径、测试目标、验收 ID 和验证器。V44 记录服务端直编、AI 消歧或历史来源；冻结 v4/v5 记录保持兼容。
...
cd opencode-loopper
./mvnw clean verify
java -jar target/opencode-loopper-0.2.60.jar
```
...
- `target/opencode-loopper-0.2.60.jar`
