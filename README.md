...
> 当前版本：`0.4.59`。默认访问 **http://127.0.0.1:8080/**，打开即进入主页。面向单机本地使用，不是多租户远程执行平台。
...
主页右上角可选择“科技蓝”或“GitHub 白”，选择立即全站生效并保存在当前浏览器，刷新与同源标签页保持一致。默认科技蓝；GitHub 白使用浅色背景、细边框与绿色主操作按钮。更多皮肤可通过[配置接口](docs/design/skins.md)扩展。
...
| 文件 | 内容 |
| --- | --- |
| `opencode-loopper-0.4.59.jar` | 后端、前端页面与 SQLite JDBC |
...
```powershell
Get-FileHash .\opencode-loopper-0.4.59.jar -Algorithm SHA256
...
```bash
java -jar opencode-loopper-0.4.59.jar
```
...
cd opencode-loopper
./mvnw clean verify
java -jar target/opencode-loopper-0.4.59.jar
```
