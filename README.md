...
> 当前版本：`0.3.12`。Loopper 默认只监听 `127.0.0.1`，面向单机本地使用，不是多租户远程执行平台。
...
- **只读 Reviewer 的确定性证据合同**：Legacy Reviewer 输出和后续 MCP 候选都必须进入同一 `ReviewerReportCompilation`。每条 finding 必须一一绑定受管相对路径、精确行号和源码 SHA-256；只要一条证据缺失、越界或不安全，整份报告失败且不保留部分 findings。全部通过后才由服务端生成 Markdown、规范 findings 和内容/源码摘要。0.3.10 只交付该确定性内核；Reviewer MCP 的持久化运行、真实模型资格和默认开启尚未完成。
- **通用 Candidate launch 协议底座**：V57 为 `REVIEWER_REPORT_V1 / PROJECT_CONVENTION_V1 / JUDGE_DECISION_V1` 增加独立的 `GENERIC_V1` 创建、提示、清理与终止持久化约束，并让 prompt dispatch 使用与既有 Acceptance `ACCEPTANCE_V55` 互斥的强类型 launch 引用。只有 `INTERNAL_MCP` 运行必须绑定 Generic launch；Legacy 双入口仍保持无 launch，三个角色的业务 Coordinator、accepted result、资格和默认开关继续分阶段接入。
...
cd opencode-loopper
./mvnw clean verify
java -jar target/opencode-loopper-0.3.12.jar
```
...
- `target/opencode-loopper-0.3.12.jar`
...
```bash
jar tf target/opencode-loopper-0.3.12.jar \
