...
> 当前版本：`0.3.20`。Loopper 默认只监听 `127.0.0.1`，面向单机本地使用，不是多租户远程执行平台。
...
- **通用 Candidate launch 协议底座**：V57 为 `REVIEWER_REPORT_V1 / PROJECT_CONVENTION_V1 / JUDGE_DECISION_V1` 增加独立的 `GENERIC_V1` 创建、提示、清理与终止持久化约束，并让 prompt dispatch 使用与既有 Acceptance `ACCEPTANCE_V55` 互斥的强类型 launch 引用。只有 `INTERNAL_MCP` 运行必须绑定 Generic launch；Legacy 双入口仍保持无 launch。Reviewer 已通过真实模型资格并默认启用；Convention 已完成接入且两次真实首投通过，但尚无“机械拒绝后同 Session 修正”证据，继续默认关闭。V63 接通双 Judge：每轮 Requirement/Risk 使用同一个不可变批次与证据代次，MCP Candidate 与 Legacy 输出共用 `JudgeDecisionCompilation`，最终 assistant text 不参与权威结论；0.3.20 将 `reason` 的 CR/LF/TAB 与普通扩展字段归为可在同一 Session 修正的机械错误，而 NUL/BEL/C1 控制字符、权限及服务端权威字段继续失败关闭；新 Judge Candidate 默认关闭，等待两个角色各自通过隔离成品 JAR 资格。
...
- **项目公约**：点击“AI 更新 Loopper 公约”会先在事务外强制刷新技术栈画像。V62 的 MCP 路线在任何远端 I/O 前冻结当前 `AGENTS.md`、技术栈指纹及组件/命令/路径证据目录；模型只能提交 `componentKeys / commandIds / pathIds` 闭集引用，不能提交原始命令、路径、权限、生命周期或最终 Markdown。Legacy Markdown 与 MCP 候选统一进入 `ProjectConventionCompilation`，由服务端确定性生成“技术栈与模块 / 构建与测试 / 目录与边界”。派发前仅确认缺少受管运行时或精确回读能力时可新建 Legacy Session；一旦可能派发，零提交、超时、传输、交互、安全、代次和停止不确定都失败关闭，最终 assistant text 不参与结算。完整预览仍须用户确认才写入，写前复核原 `AGENTS.md` 哈希和 Manifest 指纹；首次追加管理区块，后续只替换该区块，区块外人工内容始终保留。0.3.17 隔离 JAR 已两次真实首投接受，但没有机械拒绝后同 Session 修正证据，因此新 Candidate 仍默认关闭。
...
cd opencode-loopper
./mvnw clean verify
java -jar target/opencode-loopper-0.3.20.jar
```
...
- `target/opencode-loopper-0.3.20.jar`
...
```bash
jar tf target/opencode-loopper-0.3.20.jar \
