...
> 当前版本：`0.3.67`。Loopper 默认只监听 `127.0.0.1`，面向单机本地使用，不是多租户远程执行平台。
...
cd opencode-loopper
./mvnw clean verify
java -jar target/opencode-loopper-0.3.67.jar
```
...
- `target/opencode-loopper-0.3.67.jar`
...
```bash
jar tf target/opencode-loopper-0.3.67.jar \
