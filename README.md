...
> 当前版本：`0.4.63`。默认访问 **http://127.0.0.1:8080/**，打开即进入主页。面向单机本地使用，不是多租户远程执行平台。
...
| **PPT 制作** `/ppt` | 独立 PPT 助手、方案、画布、预览与可编辑 PPTX 导出 | 将资料制作成演示文稿 |
...
| 文件 | 内容 |
| --- | --- |
| `opencode-loopper-0.4.63.jar` | 后端、前端页面与 SQLite JDBC |
...
```powershell
Get-FileHash .\opencode-loopper-0.4.63.jar -Algorithm SHA256
...
```bash
java -jar opencode-loopper-0.4.63.jar
```
...
从侧栏“PPT 制作”新建作品，导入资料并说明用途。PPT 助手提出整体方向，确认方向后继续完善七个设计模块，最后点击“确认方案并开始制作”。可切换为人工制作，选择对象拖动、缩放，或在属性中修改文字和样式。局部请求先选择整份、章节、页面或对象范围；锁定内容由服务端保护。
...
cd opencode-loopper
./mvnw clean verify
java -jar target/opencode-loopper-0.4.63.jar
```
