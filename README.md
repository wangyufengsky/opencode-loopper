...
> 当前版本：`0.3.75`。Loopper 默认只监听 `127.0.0.1`，面向单机本地使用，不是多租户远程执行平台。
...
以下章节记录历史交付时的开关状态；当前默认以上述 0.3.75 为准。
...
cd opencode-loopper
./mvnw clean verify
java -jar target/opencode-loopper-0.3.75.jar
```
...
- `target/opencode-loopper-0.3.75.jar`
...
```bash
jar tf target/opencode-loopper-0.3.75.jar \
