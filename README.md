...
> 当前版本：`0.4.34`。默认访问 **http://127.0.0.1:8080/**，打开即进入主页。面向单机本地使用，不是多租户远程执行平台。
...
| 文件 | 内容 |
| --- | --- |
| `opencode-loopper-0.4.34.jar` | 后端、前端页面与 SQLite JDBC |
...
```powershell
Get-FileHash .\opencode-loopper-0.4.34.jar -Algorithm SHA256
...
```bash
java -jar opencode-loopper-0.4.34.jar
```
...
点击“开始执行”后，系统在独立目录同步并冻结 Git 证据，分批分析、校验并保存 Markdown 报告后自动完成。历史提交审查与贡献周报的新任务不强制进行最终 AI 双评审；新版代码审查逐批独立复核；历史任务继续遵守冻结的评审要求。历史提交审查输出可追溯作者与提交者的总结、问题清单及逐提交详情；新版代码审查按冻结目标版本进行功能分析和独立复核，输出当前问题、功能详情与覆盖清单，详见[代码审查合同](docs/snapshot-code-review-contract.md)；贡献周报输出总报告、每人的个人报告和内置评分排名。评分标准、原始量、去噪依据及等级理由随报告保留，详情页可预览和下载。文档生成路径和项目默认文档路径支持选择文件夹或手动输入。新模板任务暂不支持故事统计；历史已绑定任务保留统计恢复与收尾。
...
cd opencode-loopper
./mvnw clean verify
java -jar target/opencode-loopper-0.4.34.jar
```
