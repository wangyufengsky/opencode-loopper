...
> 当前版本：`0.4.66`。默认访问 **http://127.0.0.1:8080/**，打开即进入主页。面向单机本地使用，不是多租户远程执行平台。
...
| **PPT 制作** `/ppt` | 需求沟通、项目资料取材、对话修改与可编辑 PPTX 下载 | 将需求和资料制作成演示文稿 |
...
主页右上角可选择“spdb风”、“科技蓝”或“GitHub 白”，选择立即全站生效并保存在当前浏览器，刷新与同源标签页保持一致。默认 spdb风，采用浦发银行官网蓝红白风格与相关主页插画，已有手动选择继续保留；GitHub 白使用浅色背景、细边框与绿色主操作按钮。更多皮肤可通过[配置接口](docs/design/skins.md)扩展。
...
| Git | 可从终端调用；模板任务以 2.30.2 为最低基线，兼容更高版本 | 项目分支、差异与发布 |
...
| 文件 | 内容 |
| --- | --- |
| `opencode-loopper-0.4.66.jar` | 后端、前端页面与 SQLite JDBC |
...
```powershell
Get-FileHash .\opencode-loopper-0.4.66.jar -Algorithm SHA256
...
```bash
java -jar opencode-loopper-0.4.66.jar
```
...
模板任务是项目内置、可反复手动执行的公共任务。提供历史提交审查、代码审查、项目人员贡献周报，以及需求开发和需求代码评审。代码审查默认日期增量，也可选全面审查；全面审查不填写日期。项目、分支使用可搜索下拉框，默认主分支；日期使用日期选择框，默认最近七天，统一北京时间，包含起止当天。结束日期不能早于开始日期。

模板采集以 Git 2.30.2 为最低基线。日期通过完整遍历后精确筛选；文件属性从任务独立 index 读取历史 `.gitattributes`；两父合并在任务临时目录重建合并基线，只计独有解决变更。全程不依赖或改写原项目的 index、当前分支或未提交文件。分支查询和采集前检查实际 Git 版本，命令失败时按原因展示操作与退出码，不再统一归为访问权限问题。
...
点击“开始执行”后，系统在独立目录同步并冻结 Git 证据，分批分析、校验并保存 Markdown 报告后自动完成。历史提交审查与贡献周报的新任务不强制进行最终 AI 双评审；新版代码审查默认采用轻量策略，仅复核候选问题；历史任务继续遵守冻结的评审要求。历史提交审查输出可追溯作者与提交者的总结、问题清单及逐提交详情；新版代码审查对冻结目标版本按容量合并分析，关联代码在同一会话按需读取，不创建模型规划或递归补充批次，无问题结论不再独立复核，输出当前问题、功能详情与覆盖清单，详见[代码审查合同](docs/snapshot-code-review-contract.md)；贡献周报输出总报告、每人的个人报告和内置评分排名。评分标准、原始量、去噪依据及等级理由随报告保留，详情页可预览和下载。文档生成路径和项目默认文档路径支持选择文件夹或手动输入。新模板任务暂不支持故事统计；历史已绑定任务保留统计恢复与收尾。

旧模板编辑和自动化触发已停用，历史记录保留。当前只支持手动发起。执行、恢复和评分的精确定义见 [功能合同](docs/seven-feature-contract.md)。

### PPT 制作

从侧栏“PPT 制作”输入需求，可选上传资料或选择一个项目，点击“开始沟通”。选择项目后，助手可查询该项目当前可用的知识库来源。助手逐轮询问内容重点和设计偏好，已有信息不会重复问；汇总需求后，点击“确认需求，开始设计”，即可自动完成方案、制作、预览和导出。生成后直接查看预览、下载 PPTX，或继续输入“第三页简洁一些”等修改意见。需要自行调整时再打开“手动编辑”，选择对象拖动、缩放或修改文字和样式。默认修改整份作品，也可指定页面或对象；锁定内容由服务端保护。

制作结果可按页预览和检查，失败作业保留成功页面并可重试。正式导出绑定当前版本，后续修改保留旧文件。第一版包含文字、图片、图形、表格和可编辑柱状/折线/饼图；已有 PPTX 提取内容后重制。JAR 内置 Java 渲染和中文字体，无需安装 Office、LibreOffice、Node 或 Python；纯文本模型可使用结构化工具制作，视觉效果仍需结合预览检查。详细范围、字体配置及恢复语义见 [PPT 合同](docs/ppt-contract.md)。
...
- **凭据**：Provider 与 MCP Token 通过环境或内存使用，不写入数据库、日志或交付证据。数据库与 Git 账号允许受管加密保存；Git 在设置页维护全局默认账号，项目可单独覆盖，详见 [Git 账号管理](docs/git-credentials.md)。
...
cd opencode-loopper
./mvnw clean verify
java -jar target/opencode-loopper-0.4.66.jar
```
...
| [PPT 工作室合同](docs/ppt-contract.md) | PPT 工作流、工具、编辑与导出 |
...
项目页的“GitLab 与证据”可保存仓库绑定、JUnit 报告规则和应用日志来源。GitLab 沿用设置页地址与环境变量 `LOOPPER_GITLAB_PRIVATE_TOKEN`；保存后点击检查绑定，再创建新任务。日志默认限于任务工作目录；外部日志必须由人登记明确目录。配置与工具开关分开，新增工具默认停用，请在辅助 MCP 工具页逐项启用。

任务详情的“日志与测试快照”按需读取正式验证期间保存的日志和测试报告，并显示截断、来源未确认及采集失败。历史任务不追授新资源，缺少快照不表示执行成功。限制与接口见[辅助 MCP 合同](docs/assist-mcp-contract.md)。

业务超时默认关闭，可在“设置 → 全局执行上限”主动启用并填写时长；启动参数为 `LOOPPER_TIMEOUT_ENABLED`。保存仅影响新任务及新设计运行，历史任务保留原合同。数据库连接支持 JDBC URL（含多节点）与独立用户名／密码；第三方 MCP 工具可按来源批量关闭，授权边界见[辅助 MCP 合同](docs/assist-mcp-contract.md)。
