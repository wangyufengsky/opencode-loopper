...
> 当前版本：`0.3.95`。Loopper 默认只监听 `127.0.0.1`，面向单机本地使用，不是多租户远程执行平台。
...
cd opencode-loopper
./mvnw clean verify
java -jar target/opencode-loopper-0.3.95.jar
```
...
独立 JAR 可使用已安装的 JDK 21 执行 `java -jar opencode-loopper-0.3.95.jar`。如果手动生成了 Apple Silicon 或 Intel 平台包，解压后双击 `start-macos.command`，或在终端运行：
...
```bash
jar tf target/opencode-loopper-0.3.95.jar \
