...
> 当前版本：`0.3.80`。Loopper 默认只监听 `127.0.0.1`，面向单机本地使用，不是多租户远程执行平台。

## 0.3.80 回退未达标语义实验

撤回 0.3.76–0.3.79 的有限域、SAT 与独立来源复核，工作包功能恢复 0.3.75，V2 及其确定性范围修复保持开启。保留数据库迁移兼容性、历史评测证据和页面测试清理；含实验合同的数据不会被静默降级。详见 [回退范围和数据边界](docs/package-behavior-rollback.md)。
...
cd opencode-loopper
./mvnw clean verify
java -jar target/opencode-loopper-0.3.80.jar
```
...
- `target/opencode-loopper-0.3.80.jar`
