...
> 当前版本：`0.1.99`。Loopper 默认只监听 `127.0.0.1`，面向单机本地使用，不是多租户远程执行平台。
...
- **动态任务画像与专属流程**：需求分析师先给出语义标签，服务端再与有界仓库事实合并并决定流程、权限、执行与测试策略。画像按 `ROUTING / NEEDS_CONFIRMATION / CONFIRMED / FROZEN` 对外投影，所有设计入口统一依赖服务端 `confirmationReady`；完整需求触发重算时页面持续刷新，等价的人工选择会安全继承，任务意图、主要制品、单包/大型流程或读写模式发生变化时则直接展示新旧差异并要求明确确认。每次 Router 运行和完整需求快照都持久化，重启后继续轮询，旧快照在新讨论开始时终止并废弃。低于 80 分或证据冲突时不能自动继承；已明确授权的全自动模式仍不能绕过安全冲突。画像摘要及覆盖选项使用中文展示，REST/SQLite 保留稳定英文枚举码；确认后画像冻结并随 Recovery 复用。软件工作包分别冻结 Java/Python/Node/混合栈 Role Pack，Task 创建时再把 Role Pack、技术栈和测试策略复制到每个 Stage。
- **实时活动与可终止设计会话**：Designer 页面每 1.2 秒读取服务端活动快照，展示设计师增量文字、思考和普通/MCP 工具调用；任务规划师、规范工程师等结构化角色只显示工具活动和权威步骤，不显示原始规划 JSON。“清理并重新开始”先进入 `STOPPING`，停止该设计下全部远端角色 Session，全部成功后才进入 `CANCELLED` 并归档，失败时保留工作区供重试。确认设计并获得 Task 后会清除当前设计工作区并直接打开任务详情，再点左侧“设计”从新建页开始。
...
cd opencode-loopper
./mvnw clean verify
java -jar target/opencode-loopper-0.1.99.jar
```
...
5. 打开 **设计 / 循环规范**，选择项目并描述目标。软件任务默认关闭“大型任务”，因此采用一个 `WP-1`；只有确实需要多个纵向业务包时才在画像冻结前打开开关。画像计算期间会持续刷新，完成后自动沿用等价的已确认选择，或在发生实质变化时展示新旧差异和确认入口。普通模式先回答唯一一轮需求问题，服务端随后显示原样需求快照；继续补充会产生新的需求问题和替代快照，但不会启动工作包。显式确认后 WP-1 直接设计，不会再出现包级问题卡。已有未确认设计统一从 **历史设计** 页面继续、修改或归档，服务重启后会从服务端恢复同一模式和快照。
...
- `target/opencode-loopper-0.1.99.jar`
