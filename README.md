...
> 当前版本：`0.2.67`。Loopper 默认只监听 `127.0.0.1`，面向单机本地使用，不是多租户远程执行平台。
...
- **Role Pack v6、验收事实 v7 与按需 Compiler**：技术标签先归并为 Java、Python、Node 和 Other 软件族，JUnit/Jupiter/Surefire 等测试标签不会把 Java 任务误判为混合栈，工作包识别使用技术词边界，`ChainNodeInvoker` 等业务符号不会误触发 Node。Designer 继续只写受控 Markdown，并用“阶段 / 目标 / 包含场景/评审/交付 / 前置阶段”精确引用前文标题；服务端只做 NFKC、空白和大小写规范化，不删除标点或模糊匹配。新软件包把冻结需求与正向设计交付中的显式仓库相对必改路径连同来源和 SHA-256 写入 v7 不可变事实快照；在 lowering 前逐项证明它们进入由 Stage 自己引用的受控交付/范围事实形成的路径合同，并与 focused test、`GIT_DIFF` 共用允许/禁止集合。包级范围、全局事实和技术栈 fallback 不能冒充归属证明。遗漏路径、删除、移动源端、正负冲突或禁区交集直接形成具体 `DESIGN_INCOMPLETE`；项目根外路径在创建编译记录前收束，不会被误当成模型传输失败。上述缺口不会等执行期才以 `outside allowed paths` 失败，也不会放宽运行期验证器。完整普通 `WP-1` 直接按 Designer 的 1–6 个阶段完成能力求解、EARS 条件与 LoopSpec v2 lowering，不创建 Compiler Session 或消耗模型调用。只有闭集事实/能力仍有歧义时才调用一次无工具 Compiler 填洞，继续使用 V6 小型消歧 Schema；大型任务也只调用一次并锁定阶段拓扑。非法输出定点退回完整设计，不丢弃为空建议或降级成通用单阶段。V44 记录服务端直编、AI 消歧或历史来源；冻结 v4/v5 及 v6 事实快照保持原语义兼容。
...
cd opencode-loopper
./mvnw clean verify
java -jar target/opencode-loopper-0.2.67.jar
```
...
- `target/opencode-loopper-0.2.67.jar`
