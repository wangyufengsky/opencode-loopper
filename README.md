...
> 当前版本：`0.4.64`。默认访问 **http://127.0.0.1:8080/**，打开即进入主页。面向单机本地使用，不是多租户远程执行平台。
...
| **PPT 制作** `/ppt` | 一句话生成、对话修改、可选手动编辑与可编辑 PPTX 下载 | 将需求和资料制作成演示文稿 |
...
| 文件 | 内容 |
| --- | --- |
| `opencode-loopper-0.4.64.jar` | 后端、前端页面与 SQLite JDBC |
...
```powershell
Get-FileHash .\opencode-loopper-0.4.64.jar -Algorithm SHA256
...
```bash
java -jar opencode-loopper-0.4.64.jar
```
...
从侧栏“PPT 制作”输入需求，可选上传资料，点击“生成 PPT”。助手自行设计内容和版式并制作完整演示文稿；只有缺少关键信息时才简短提问，回答后继续生成。生成后直接查看预览、下载 PPTX，或继续输入“第三页简洁一些”等修改意见。需要自行调整时再打开“手动编辑”，选择对象拖动、缩放或修改文字和样式。默认修改整份作品，也可指定页面或对象；锁定内容由服务端保护。
...
cd opencode-loopper
./mvnw clean verify
java -jar target/opencode-loopper-0.4.64.jar
```
