...
is_startup_key() {
  case "$1" in
    ...
    LOOPPER_TIMEOUT_ENABLED|LOOPPER_MAX_TASK_ATTEMPTS|LOOPPER_TEMPLATE_ANALYSIS_CONCURRENCY|LOOPPER_SESSION_ERROR_LIMIT|LOOPPER_MAX_DURATION|LOOPPER_ATTEMPT_TIMEOUT|\
...
validate_startup_value() {
  ...
  case "${key}" in
    SERVER_PORT|LOOPPER_ABORT_CLEANUP_ATTEMPTS|LOOPPER_MAX_STAGE_ATTEMPTS|LOOPPER_MAX_TASK_ATTEMPTS|LOOPPER_TEMPLATE_ANALYSIS_CONCURRENCY|LOOPPER_SESSION_ERROR_LIMIT)
      [[ "${value}" =~ ^[0-9]+$ ]] ;;
    LOOPPER_OPEN_BROWSER|OPENCODE_ENABLE_QUESTION_TOOL|LOOPPER_TIMEOUT_ENABLED) [[ "${value}" == "true" || "${value}" == "false" ]] ;;
...
elif [[ -f "${APP_HOME}/target/opencode-loopper-0.4.30.jar" ]]; then
  JAR_PATH="${APP_HOME}/target/opencode-loopper-0.4.30.jar"
elif [[ -f "${APP_HOME}/opencode-loopper-0.4.30.jar" ]]; then
  JAR_PATH="${APP_HOME}/opencode-loopper-0.4.30.jar"
else
  fail "找不到成品 JAR。请把 opencode-loopper-0.4.30.jar 放到 ${APP_HOME}，或设置 LOOPPER_JAR_PATH。"
fi
