...
elif [[ -f "${APP_HOME}/target/opencode-loopper-0.2.73.jar" ]]; then
  JAR_PATH="${APP_HOME}/target/opencode-loopper-0.2.73.jar"
elif [[ -f "${APP_HOME}/opencode-loopper-0.2.73.jar" ]]; then
  JAR_PATH="${APP_HOME}/opencode-loopper-0.2.73.jar"
else
  fail "找不到成品 JAR。请把 opencode-loopper-0.2.73.jar 放到 ${APP_HOME}，或设置 LOOPPER_JAR_PATH。"
fi
