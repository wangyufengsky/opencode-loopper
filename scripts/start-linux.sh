...
is_startup_key() {
  case "$1" in
    ...
    LOOPPER_TIMEOUT_ENABLED|LOOPPER_MAX_TASK_ATTEMPTS|LOOPPER_SESSION_ERROR_LIMIT|LOOPPER_MAX_DURATION|LOOPPER_ATTEMPT_TIMEOUT|\
...
elif [[ -f "${APP_HOME}/target/opencode-loopper-0.4.26.jar" ]]; then
  JAR_PATH="${APP_HOME}/target/opencode-loopper-0.4.26.jar"
elif [[ -f "${APP_HOME}/opencode-loopper-0.4.26.jar" ]]; then
  JAR_PATH="${APP_HOME}/opencode-loopper-0.4.26.jar"
else
  fail "找不到成品 JAR。请把 opencode-loopper-0.4.26.jar 放到 ${APP_HOME}，或设置 LOOPPER_JAR_PATH。"
fi
