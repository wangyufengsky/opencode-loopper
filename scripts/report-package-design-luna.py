...
def aggregate(corpus, roots):
    ...
    for root in roots:
        for path in sorted(Path(root).rglob('run.json')):
            ...
            correct = reviewed and review.get('candidateSha256')==candidate_hash and review.get('scopeVerdict')=='SUPPORTED' and all(item.get('verdict')=='SUPPORTED' and item.get('evidence') for item in checks.values())
            ...
            rows.append(dict(caseId=case['id'], family=case['family'], split=case['split'], expectedClass=case['expectedClass'],
                ...
                preparationTurns=run.get('semanticPreparationTurns',0), elapsedSeconds=run['elapsedSeconds'],
                turnUsage=run.get('turnUsage',[]), fixtureUnchanged=run.get('fixtureUnchanged',False),
                actualModelRequests=None))
    return rows
...
def metrics(rows):
    ...
    return dict(runs=len(rows), resolvableRuns=len(resolvable),
        ...
        preparationTurns=sum(r['preparationTurns'] for r in rows), repeatedIssues=sum(r['repeatedIssues'] for r in rows),
        ...
        actualModelRequests=None, fixtureChanges=sum(not r['fixtureUnchanged'] for r in rows))
