...
if exist "%APP_HOME%\target\opencode-loopper-0.3.80.jar" (
  set "JAR_PATH=%APP_HOME%\target\opencode-loopper-0.3.80.jar"
  goto jar_ready
)
if exist "%APP_HOME%\opencode-loopper-0.3.80.jar" (
  set "JAR_PATH=%APP_HOME%\opencode-loopper-0.3.80.jar"
  goto jar_ready
)
goto jar_missing
...
:jar_missing
echo [Loopper] ERROR: opencode-loopper-0.3.80.jar was not found under "%APP_HOME%". Put the release JAR beside this script or set LOOPPER_JAR_PATH. 1>&2
exit /b 1
