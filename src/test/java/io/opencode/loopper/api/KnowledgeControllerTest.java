...
class KnowledgeControllerTest {
    @Test void writeAuthorizationRunsBeforeAnyModelOrSourceOperation() throws Exception {
        ...
        var mvc=MockMvcBuilders.standaloneSetup(new KnowledgeController(conversations,coordinator,new KnowledgeEventHub()),new KnowledgeSourceController(sources,reader,databases)).setControllerAdvice(new ApiExceptionHandler()).build();
        ...
        mvc.perform(post("/api/projects/p/knowledge-sources/directories").contentType("application/json").content("{\"path\":\"/tmp\"}"))
            .andExpect(status().isBadRequest());
        verifyNoInteractions(conversations,coordinator,sources,reader,databases);
    }
}
