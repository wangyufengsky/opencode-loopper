...
class KnowledgeMigrationTest {
    ...
    @Test void upgradesCurrentReleaseWithoutConvertingDesignsAndKeepsExistingPolicies() throws Exception {
        ...
        var flyway = Flyway.configure().dataSource(url,null,null).load(); assertThat(flyway.migrate().migrationsExecuted).isEqualTo(26); flyway.validate();
        try (var connection=DriverManager.getConnection(url);var sql=connection.createStatement()) {
    ...
    @Test void upgradesExistingKnowledgeAnswersWithEmptyThinkingWithoutRewritingHistory() throws Exception {
        ...
        var flyway = Flyway.configure().dataSource(url,null,null).load(); assertThat(flyway.migrate().migrationsExecuted).isEqualTo(25); flyway.validate();
    ...
    @Test void researchUpgradePreservesFrozenV2HistoryAndAddsEmptyContinuationStore() throws Exception {
        ...
        var flyway = Flyway.configure().dataSource(url,null,null).load(); assertThat(flyway.migrate().migrationsExecuted).isEqualTo(22); flyway.validate();
        try (var connection=DriverManager.getConnection(url);var sql=connection.createStatement()) {
    ...
    @Test void v2UpgradeFreezesLegacyContractAndPreservesCurrentAnswerAndRevokedGitPolicy() throws Exception {
        ...
        var flyway = Flyway.configure().dataSource(url,null,null).load(); assertThat(flyway.migrate().migrationsExecuted).isEqualTo(23); flyway.validate();
        try (var connection=DriverManager.getConnection(url);var sql=connection.createStatement()) {
