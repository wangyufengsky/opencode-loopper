-- Extend closed candidate kinds for direct source review; preserve all prior ownership guards.
PRAGMA foreign_keys=OFF;
PRAGMA legacy_alter_table=ON;
SAVEPOINT candidate_submission_v108;
ALTER TABLE ai_candidate_submission_run RENAME TO ai_candidate_submission_run_v107;
...
CREATE TABLE ai_candidate_submission_run (
    ...
    owner_type TEXT NOT NULL CHECK (owner_type IN (
        ...
        'TASK_PACKAGE_PLAN_REVISION','ANALYSIS_REPORT','PROJECT_CONVENTION_DRAFT','JUDGE_RUN','DOCUMENT_TEMPLATE_MODEL_RUN')),
...
CREATE TRIGGER trg_candidate_owner_scope_v108_copy
...
INSERT INTO ai_candidate_submission_run SELECT * FROM ai_candidate_submission_run_v107;
DROP TRIGGER trg_candidate_owner_scope_v108_copy;
DROP TABLE ai_candidate_submission_run_v107;
...
CREATE TEMP TABLE candidate_submission_v108_fk_guard (violations INTEGER CHECK (violations=0));
INSERT INTO candidate_submission_v108_fk_guard SELECT COUNT(*) FROM pragma_foreign_key_check
  WHERE "table"='ai_candidate_submission_run' OR parent='ai_candidate_submission_run';
DROP TABLE candidate_submission_v108_fk_guard;
RELEASE candidate_submission_v108;
