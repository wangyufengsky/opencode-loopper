-- Add source-template roles and preserve every existing candidate ownership and settlement guard.
PRAGMA foreign_keys=OFF;
PRAGMA legacy_alter_table=ON;
SAVEPOINT candidate_submission_v129;
ALTER TABLE ai_candidate_submission_run RENAME TO ai_candidate_submission_run_v128;
...
CREATE TABLE ai_candidate_submission_run (
    ...
    owner_type TEXT NOT NULL CHECK (owner_type IN (
        ...
        'TASK_PACKAGE_PLAN_REVISION','ANALYSIS_REPORT','PROJECT_CONVENTION_DRAFT','JUDGE_RUN','DOCUMENT_TEMPLATE_MODEL_RUN','SOURCE_TEMPLATE_MODEL_RUN')),
    ...
    candidate_kind TEXT NOT NULL CHECK (candidate_kind IN (
        ...
        'ROLLING_PACKAGE_PLAN_V1','REVIEWER_REPORT_V1','PROJECT_CONVENTION_V1','JUDGE_DECISION_V1','DOCUMENT_REQUIREMENTS_V1','DOCUMENT_REQUIREMENT_REVIEW_V1','REQUIREMENT_CODE_ASSESSMENT_V1','REQUIREMENT_ASSESSMENT_REVIEW_V1','DOCUMENT_CODE_ASSESSMENT_V2','DOCUMENT_CODE_REVIEW_V2','SOURCE_DETAILED_DESIGN_V1','SOURCE_DESIGN_REVIEW_V1')),
    ...
    correction_limit INTEGER CHECK (correction_limit IS NULL OR
        (correction_limit BETWEEN 2 AND 16 AND attempts_used <= correction_limit AND submission_channel='INTERNAL_MCP')),
    ...
    CHECK (
        (candidate_kind IN ('SOURCE_DETAILED_DESIGN_V1','SOURCE_DESIGN_REVIEW_V1') AND owner_type='SOURCE_TEMPLATE_MODEL_RUN' AND project_id IS NOT NULL)
        OR
        (candidate_kind IN ('DOCUMENT_REQUIREMENTS_V1','DOCUMENT_REQUIREMENT_REVIEW_V1','REQUIREMENT_CODE_ASSESSMENT_V1','REQUIREMENT_ASSESSMENT_REVIEW_V1','DOCUMENT_CODE_ASSESSMENT_V2','DOCUMENT_CODE_REVIEW_V2') AND owner_type='DOCUMENT_TEMPLATE_MODEL_RUN' AND project_id IS NOT NULL)
        OR
...
CREATE TRIGGER trg_candidate_owner_scope_v129_copy
BEFORE INSERT ON ai_candidate_submission_run
BEGIN
    SELECT CASE
        WHEN NEW.owner_type='SOURCE_TEMPLATE_MODEL_RUN' AND NOT EXISTS (
            SELECT 1 FROM source_template_model model JOIN source_template_run owner ON owner.id=model.run_id
            WHERE model.id=NEW.owner_id AND owner.project_id=NEW.project_id
              AND model.candidate_kind=NEW.candidate_kind AND model.external_session_id=NEW.external_session_id)
            THEN RAISE(ABORT,'candidate owner scope mismatch')
        WHEN NEW.owner_type='DOCUMENT_TEMPLATE_MODEL_RUN' AND NOT EXISTS (
            SELECT 1 FROM document_template_model_run model JOIN document_template_run owner ON owner.id=model.run_id
            WHERE model.id=NEW.owner_id AND owner.project_id=NEW.project_id
              AND model.candidate_kind=NEW.candidate_kind AND model.external_session_id=NEW.external_session_id)
            THEN RAISE(ABORT,'candidate owner scope mismatch')
...
INSERT INTO ai_candidate_submission_run SELECT * FROM ai_candidate_submission_run_v128;
DROP TRIGGER trg_candidate_owner_scope_v129_copy;
DROP TABLE ai_candidate_submission_run_v128;
...
CREATE TEMP TABLE candidate_submission_v129_fk_guard (violations INTEGER CHECK (violations=0));
INSERT INTO candidate_submission_v129_fk_guard SELECT COUNT(*) FROM pragma_foreign_key_check
  WHERE "table"='ai_candidate_submission_run' OR parent='ai_candidate_submission_run';
DROP TABLE candidate_submission_v129_fk_guard;
RELEASE candidate_submission_v129;
