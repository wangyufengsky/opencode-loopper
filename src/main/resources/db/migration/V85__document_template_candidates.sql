-- Add document-template candidate ownership while retaining every existing candidate guard.
PRAGMA foreign_keys=OFF;
PRAGMA legacy_alter_table=ON;
SAVEPOINT candidate_submission_v85;
...
CREATE TABLE document_template_model_run (
 ...
 UNIQUE(run_id,candidate_kind,ordinal,generation)
);
CREATE INDEX idx_document_template_model_active ON document_template_model_run(run_id,state,id);


ALTER TABLE ai_candidate_submission_run RENAME TO ai_candidate_submission_run_v84;
...
CREATE TABLE ai_candidate_submission_run (
    ...
    candidate_kind TEXT NOT NULL CHECK (candidate_kind IN (
        ...
        'ROLLING_PACKAGE_PLAN_V1','REVIEWER_REPORT_V1','PROJECT_CONVENTION_V1','JUDGE_DECISION_V1','DOCUMENT_REQUIREMENTS_V1','DOCUMENT_REQUIREMENT_REVIEW_V1','REQUIREMENT_CODE_ASSESSMENT_V1','REQUIREMENT_ASSESSMENT_REVIEW_V1')),
    ...
    CHECK (
        (candidate_kind IN ('DOCUMENT_REQUIREMENTS_V1','DOCUMENT_REQUIREMENT_REVIEW_V1','REQUIREMENT_CODE_ASSESSMENT_V1','REQUIREMENT_ASSESSMENT_REVIEW_V1') AND owner_type='DOCUMENT_TEMPLATE_MODEL_RUN' AND project_id IS NOT NULL)
        OR
...
CREATE TRIGGER trg_candidate_owner_scope_v85_copy
...
INSERT INTO ai_candidate_submission_run SELECT * FROM ai_candidate_submission_run_v84;
DROP TRIGGER trg_candidate_owner_scope_v85_copy;
DROP TABLE ai_candidate_submission_run_v84;
...
CREATE TEMP TABLE candidate_submission_v85_fk_guard (violations INTEGER CHECK (violations=0));
INSERT INTO candidate_submission_v85_fk_guard SELECT COUNT(*) FROM pragma_foreign_key_check
  WHERE "table"='ai_candidate_submission_run' OR parent='ai_candidate_submission_run';
DROP TABLE candidate_submission_v85_fk_guard;
RELEASE candidate_submission_v85;
