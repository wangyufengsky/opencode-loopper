...
public class InsightReadService {
    ...
    public InsightReadService(ReadModelMapper mapper, MeterRegistry metrics) {
        this.mapper = mapper;
    ...
    public InsightPage page(String cursor, Integer requestedLimit) {
        return metrics.timer("loopper.read_model.duration", "model", "insight.page").record(() -> {
            ...
            List<TaskInsightRow> rows = mapper.taskInsightsPage(decoded == null ? null : decoded.value(),
            ...
            Map<String, Map<String, String>> costs = costs(ids.isEmpty() ? List.of() : mapper.usageCosts(ids));
            List<TaskInsight> items = pageRows.stream().map(row -> insight(row, costs.getOrDefault(row.id(), Map.of()))).toList();
            GlobalUsageRow global = mapper.globalUsage();
            Map<String, String> globalCosts = mergeCosts(mapper.usageCosts(null));
    ...
    private TaskInsight insight(TaskInsightRow row, Map<String, String> costs) {
        ...
        String qualityState = !deterministic ? "PENDING" : requirement && risk ? "PASS" : "REVIEW_REQUIRED";
    ...
    public record Quality(boolean deterministicPassed, int verificationCount, int verificationPassedCount,
                          boolean requirementJudgePassed, boolean riskJudgePassed, String state) { }
}
