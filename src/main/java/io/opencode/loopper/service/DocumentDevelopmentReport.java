...
public final class DocumentDevelopmentReport {
    ...
    public DocumentDevelopmentReport(DocumentDevelopmentEvidence evidence, DocumentRequirementMapper requirements,
            DocumentTemplateMapper runs, DocumentRequirementReportService reports, ObjectMapper json, DocumentRequirementClarifications clarifications) {
        this.evidence = evidence; this.requirements = requirements; this.runs = runs; this.reports = reports; this.json = json; this.clarifications = clarifications;
    }
    public void render(DocumentTemplateRunRow run) {
        ...
        var snapshot = evidence.read(run); var matrix = new StringBuilder("# 需求验收矩阵\n\n| 需求 | 验收结果 | 对应阶段 |\n|---|---|---|\n");
        ...
        var answers = clarifications.answers(run.id(), run.requirementRevision());
        ...
        reports.save(run, "matrix.json", "REQUIREMENT_MATRIX", json.writeValueAsString(Map.of(
                "templateVersion", run.templateVersion(), "requirementRevision", run.requirementRevision(),
