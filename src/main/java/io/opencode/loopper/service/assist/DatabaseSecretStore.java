...
/** Versioned AES-GCM files; plaintext and master key never enter persistence DTOs. */
@Component
public class DatabaseSecretStore {
    ...
    DatabaseSecretStore(Path directory,Path keyFile,String environmentKey) {
        this.directory=directory.toAbsolutePath().normalize(); this.keyFile=keyFile.toAbsolutePath().normalize();
    ...
    static void secureDirectory(Path directory) throws Exception {
    ...
    static void requireRegular(Path file) throws Exception {
    ...
    private static AssistFailure failure() {
