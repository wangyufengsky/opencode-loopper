...
final class DocumentRequirementContext {
    ...
    static String index(DocumentTemplateRunRow run, String manifest, int count) {
        return INDEX + JSON.writeValueAsString(Map.of("runId", run.id(), "revision", run.requirementRevision(),
                "manifestSha256", manifest, "requirementCount", count))
                + "\n软件开发范围由冻结需求清单定义，使用专属 MCP 分页读取每项需求和原文；标题不替代需求正文。"
    ...
    static String planPrompt(DocumentDesignContextMapper mapper, String planId, String fallback) {
        return mapper.documentPlanSource(planId).map(source -> INDEX + JSON.writeValueAsString(Map.of("runId", source.runId(),
                ...
                + "\n使用冻结需求专属 MCP 读取本版全部需求及原文，核对已冻结事实，仅调整剩余包并保持需求覆盖；最后一包须包括整体回归。")
                .orElse(fallback);
    }
    ...
    static String text(String source) {
        ...
        return envelope(source).entries().stream().map(entry -> entry.title() + "\n" + entry.statement()
    ...
    static String prompt(String text) {
        ...
        return "冻结文档需求 run=" + source.runId() + ", revision=" + source.revision() + ", sha256=" + source.manifestSha256()
                + "。本包及全局约束来源共 " + source.entries().size() + " 项，sourceRefs 使用对应的 RQ 编号。\n"
                ...
                + "\n使用 list_development_requirements 分页定位，read_development_requirement 逐项读取正文、验收和引用；"
