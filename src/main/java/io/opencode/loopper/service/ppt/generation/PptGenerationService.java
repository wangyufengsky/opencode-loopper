...
public class PptGenerationService {
    ...
    public PptGenerationService(PptGenerationMapper mapper,PptGenerationPersistence persistence,PptGenerationCoordinator coordinator,
            PptDocuments documents,PptAgentService agent,PptAgentMapper agents,ObjectMapper json) {
        this.mapper=mapper;this.persistence=persistence;this.coordinator=coordinator;this.documents=documents;
        this.agent=agent;this.agents=agents;this.json=json;
    }
    ...
    public record View(String id,String documentId,String state,String step,String detail,long revision,long version,
            String runId,String jobId,boolean canResume,String createdAt,String updatedAt) { }
    public View generate(String document,Generate input) {
        ...
        var row=persistence.begin(row(document,input.idempotencyKey(),sha,input.prompt(),input.expectedRevision(),"CREATE",json.writeValueAsString(Map.of("kind","DOCUMENT"))));
    ...
    public PptAgentService.Message send(String document,PptAgentService.Send input) {
        ...
        var row=persistence.begin(row(document,input.idempotencyKey(),sha,input.text(),input.expectedRevision(),"REVISE",json.writeValueAsString(scope)));
    ...
    private Generation row(String document,String key,String sha,String prompt,long revision,String mode,String scope) {
        ...
        return new Generation(id,document,key,sha,prompt,mode,scope,revision,revision,step,step,0,
                PptGenerationPersistence.agentKey(id,0,step),null,null,null,null,"",0,now,now);
    }
    private View view(Generation row) {
        ...
        return new View(row.id(),row.documentId(),row.state(),row.step(),row.detail(),doc.revision(),row.version(),row.runId(),row.jobId(),
                Set.of("STOPPED","FAILED").contains(row.state())&&!doc.archived()&&agents.activeCount(row.documentId())==0,row.createdAt(),row.updatedAt());
    }
