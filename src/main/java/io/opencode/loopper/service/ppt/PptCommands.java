...
public class PptCommands {
    ...
    private final io.opencode.loopper.service.ppt.agent.PptAgentWorkflowGate workflow;
    public PptCommands(PptDocuments documents,PptAgentService agent,ObjectMapper json,io.opencode.loopper.service.ppt.agent.PptAgentWorkflowGate workflow){this.documents=documents;this.agent=agent;this.json=json;this.workflow=workflow;}
    public PptDocuments.View action(String id,String action,PptDocuments.Action input) {
        ...
        documents.action(id,action,input,()->{workflow.assertManualAdmission(id);agent.assertNoActiveWriter(id);});
