...
final class PackageRequirementSources {
    ...
    static Map<String, Source> index(String original) {
        var sourceTemplate = SourceRequirementContext.sources(original);
        if (sourceTemplate != null) return sourceTemplate;
    ...
    static String prompt(String original) {
        if (SourceRequirementContext.source(original)) return SourceRequirementContext.prompt(original);
