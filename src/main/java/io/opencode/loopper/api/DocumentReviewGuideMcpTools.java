...
final class DocumentReviewGuideMcpTools {
    ...
    static List<McpServerFeatures.SyncToolSpecification> specifications(DocumentReviewGuideService service,
        ...
        return List.of(tool("get_document_review_work", "List this batch's original headings, read receipts and existing requirement keys. Read the actual original sections before assessing.",
                ...
                args -> service.work((String) args.get("runId"), integer(args, "offset"), integer(args, "limit")), authority, service, json),
                tool("check_document_review_candidate", "Read-only candidate preflight. Returns exact validation problems, never accepts or consumes a candidate submission. Fix problems before checking again.",
                        Map.of("runId", text, "candidate", Map.of("anyOf", List.of(candidateSchema(io.opencode.loopper.domain.MachineCandidateKind.DOCUMENT_CODE_ASSESSMENT_V2), candidateSchema(io.opencode.loopper.domain.MachineCandidateKind.DOCUMENT_CODE_REVIEW_V2)))), args -> service.check((String) args.get("runId"), candidate(args)), authority, service, json));
    }
    ...
    private static McpServerFeatures.SyncToolSpecification tool(String name, String description, Map<String, Object> properties,
            Function<Map<String, Object>, Map<String, Object>> action, InternalRoleToolAuthority authority,
            DocumentReviewGuideService service, ObjectMapper json) {
        ...
        return McpServerFeatures.SyncToolSpecification.builder().tool(tool).callHandler((exchange, request) -> {
            try {
                ...
                authority.require(service.authorizedSession(id), name);
