...
public class RuntimeController {
    ...
    private static RuntimeDto dto(OpenCodeRuntimeManager.RuntimeSnapshot snapshot) {
        return new RuntimeDto(snapshot.status(), snapshot.version(), snapshot.managed(), snapshot.pid(), snapshot.endpoint(), snapshot.model(), snapshot.checkedAt().toString());
    }
    public record RuntimeDto(String status, String version, boolean managed, Long pid, String endpoint, String model, String checkedAt) { }
}
