...
public class LoopperMcpTools {
    ...
    public Map<String, Object> proposeLoopSpec(
            ...
            @ToolParam(description = "Complete LoopSpec v2 to persist unchanged", required = true) LoopSpec spec) {
        ...
        var designerSession = requireReadOnlyDesigner(sessionId, normalizedProjectId);
        ...
        boolean persistedLegacy = designerSession.loopDraftId() != null
                && "v1".equals(drafts.spec(drafts.get(designerSession.loopDraftId())).schemaVersion());
        var assessment = drafts.assessment(spec, true, persistedLegacy);
        if (!assessment.valid()) throw new BadRequestException("LOOPSPEC_INVALID", String.join("; ", assessment.errors()));
    ...
    @Tool(name = "validate_loop_spec", description = "Validate a new LoopSpec v2, or validate a persisted v1/v2 draftId paired with its exact version")
    public Map<String, Object> validateLoopSpec(
        ...
        if (draftId != null && !draftId.isBlank()) {
            ...
            return validationResult(drafts.spec(draft), draft.id(), version, true);
        }
        if (spec == null) return Map.of("valid", false, "errors", List.of("spec or draftId/version is required"));
        return validationResult(spec, null, null, false);
    }

    @Tool(name = "create_task", description = "Create or return the one task for a CONFIRMED draft; switch the registered checkout to a serialized Task branch when Git HEAD exists, otherwise use the registered directory directly; this never auto-confirms a draft")
    ...
    private Map<String, Object> validationResult(LoopSpec spec, String draftId, Long version, boolean allowLegacy) {
        var assessment = drafts.assessment(spec, true, allowLegacy);
        Map<String, Object> result = new java.util.LinkedHashMap<>();
        result.put("valid", assessment.valid());
        result.put("schemaVersion", assessment.schemaVersion());
        result.put("legacy", assessment.legacy());
        result.put("errors", assessment.errors());
        result.put("stageAssessments", assessment.stageAssessments());
        if (draftId != null) {
    ...
    private io.opencode.loopper.persistence.DesignerSessionRow requireReadOnlyDesigner(String designerSessionId, String projectId) {
