...
final class DocumentDevelopmentMcpTools {
    ...
    static List<McpServerFeatures.SyncToolSpecification> specifications(DocumentDevelopmentReads reads,
            InternalRoleToolAuthority authority, ObjectMapper json) {
        return List.of(spec("get_development_task_guide", "Read this role's frozen project directory, stage objective, allowed paths and deliverables before planning or editing", reads, authority, json, Map.of("scope", text())),spec("list_development_documents", "Read the immutable document index, source references and resource URIs", reads, authority, json, Map.of("scope", text())),
                spec("list_development_sections", "Read a page of original document headings, not inferred requirements", reads, authority, json,
                        Map.of("scope", text(), "fileId", text(), "offset", Map.of("type", "integer", "minimum", 0))),
                spec("list_development_requirements", "Page through immutable requirements and source identities for the signed software role", reads, authority, json,
                        Map.of("scope", text(), "after", Map.of("type", "integer", "minimum", -1))),
                spec("read_development_requirement", "Read one complete frozen requirement, acceptance scenarios and source references", reads, authority, json,
                        Map.of("scope", text(), "key", text())),
                spec("read_development_source", "Read one frozen document segment using its expected document hash", reads, authority, json,
    ...
    private static McpServerFeatures.SyncToolSpecification spec(String name, String description, DocumentDevelopmentReads reads,
        ...
        return McpServerFeatures.SyncToolSpecification.builder().tool(tool).callHandler((exchange, request) -> {
            try {
                ...
                authority.require(reads.authorizedSession(grant), name);
