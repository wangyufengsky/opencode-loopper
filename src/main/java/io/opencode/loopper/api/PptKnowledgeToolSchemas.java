...
final class PptKnowledgeToolSchemas {
    ...
    static void parameters(String tool,Map<String,Object> args,List<String> required) {
        switch(tool) {
            ...
            case "ppt_read_knowledge_source" -> {
                ...
                integer(args,"section",-1,10000);integer(args,"startLine",1,Integer.MAX_VALUE);integer(args,"endLine",0,Integer.MAX_VALUE);integer(args,"offset",0,10000);
            }
    ...
    static String description(String tool) {
        return switch(tool) {
            ...
            case "ppt_read_knowledge_source" -> "Read a saved evidenceId from this PPT's plan sourceIds to recover its original captured text, or read current sourceId with relative path. Provide evidenceId OR sourceId. For documents omit section to list sections, then read section. Optional startLine/endLine/expectedSha/offset. New saved reads return evidenceId usable in PPT sourceIds.";
