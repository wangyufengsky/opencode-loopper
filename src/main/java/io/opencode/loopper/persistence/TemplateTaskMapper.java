...
public interface TemplateTaskMapper {
    ...
    @Insert("""
            INSERT INTO template_task_batch(id,task_id,attempt_id,session_id,ordinal,purpose,input_json,input_sha256,state,
                creation_plan_json,prompt_json,prompt_sha256,output_json,error_code,error_message,created_at,updated_at,version)
            VALUES(#{id},#{taskId},#{attemptId},#{sessionId},#{ordinal},#{purpose},#{inputJson},#{inputSha256},#{state},
                #{creationPlanJson},#{promptJson},#{promptSha256},#{outputJson},#{errorCode},#{errorMessage},#{createdAt},#{updatedAt},#{version})
            """)
    ...
    @Select("SELECT * FROM template_task_batch WHERE task_id=#{taskId} AND attempt_id=#{attemptId} AND purpose=#{purpose} AND ordinal=#{ordinal}")
