...
11. **AI 与程序交互优先 MCP**：新增或扩展功能涉及 AI 查询业务数据、提交候选或请求程序动作时，优先使用职责明确、结构化的 MCP 工具，避免把解析最终自然语言或约定文本标记作为默认业务协议。工具应将校验失败的具体原因和可执行修正指令返回 AI，让可修正问题在同一会话内反馈、修正、重交；写入类交互保持身份、版本、幂等与权限校验，MCP 调用成功不替代服务端业务验收。确需其他通道时，在对应功能合同说明原因及反馈方式；历史任务仍遵守冻结协议。通道选择不隐含单次输出额度、会话轮数或任务总预算的限制，这些策略应分别定义。
