...
当前交付版本：`0.4.56`。实际版本由 `pom.xml` 持有，发布引用用脚本同步。
...
- 推送分支、创建 PR/MR、发布标签/Release 和远端部署分别需要对应授权。普通代码交付不包含这些动作；“发版/推送新版本”包含标准提交、标签与 Release 流程。不得强推、移动或复用正式标签。
- 每次更新版本并通过完整 JAR 门禁后，必须检查当前环境中的本项目 Loopper 服务：没有运行实例则启动本次新版本，有则替换为本次新版本，验证实际运行成功后再向用户交付。这是持续授权，无需逐次确认；用户明确要求不启动或不替换时除外。保留数据与配置、备份和停止证明，具体流程见 [本地服务更新](docs/development.md#版本更新后的本地服务)。
- 本地服务更新仅限已确认归属的本项目实例，不停止无关 Java 服务或外部 OpenCode。隔离验收使用独立数据、端口和进程，清理本次创建的临时进程；交付服务保持运行。真实 Provider 调用遵守任务授权和预算。
...
11. **AI 与程序交互优先 MCP**：AI 查询业务数据、提交候选或请求程序动作时，优先使用结构化 MCP；写入保持身份、版本、幂等和权限校验，工具成功不替代业务验收。反馈修正、通道例外、历史兼容和独立预算见 [OpenCode 合同](docs/opencode-contract.md#ai-与程序交互的新增和扩展)。

## 3. 工作方式与完成条件

1. 识别本次目标、变更前状态、影响范围与完成证据。行为或结构修改前阅读相关合同、目标源码和相邻测试；新增实现模式时优先参考现有相似实现。纯文案、排版和局部解释只读判断与验证所需内容；跨模块、生命周期、权限或兼容变化沿受影响链路补齐上下文，不强制建立全仓库地图。
2. 对相关问题集中定位根因和受影响路径，再形成一致的修改批次；实施期间可运行必要的聚焦测试取得反馈。
3. 按需补充行为测试，复用既有夹具。纯文案、排版或无行为变化的低影响修改不要求镜像实现的测试；说明实际采用的检查。
4. 按下表选择验证级别。需要完整门禁时，先完成聚焦与受影响集成回归，再对稳定候选执行一次完整门禁。通过后继续完成适用的运行验证、交付和提交，不停在首版实现；仅因输入变化、新失败或具体未解决风险补充验证。
...
完整交付：保护既有运行产物 → 同步一个未使用且递增的版本 → `./scripts/verify.sh` → 检查正式 JAR、静态资源和 SHA-256 → 检查并启动或替换当前环境的本项目服务 → 验证实际运行 → 更新交付记录 → 本地提交 → 回复用户。只回填测试数、哈希和结果不触发递归打包；纯说明文档不触发版本或服务更新。聚焦失败先修复；完整候选构建后若可执行交付内容再变，重新分配版本。保留失败输出，不伪造成功。

最终回复只报告本次适用的证据：改动、验证及未覆盖项、本地提交；更新版本时报告 JAR 路径/哈希、启动或替换结果、实际运行版本/PID、访问地址和数据备份位置；收到发布授权时报告标签、Actions 与远端资产。未完成的服务更新及阻塞原因必须明确，不将构建成功当作运行成功。源码、测试、JAR、活动 JVM、浏览器与 Provider 是独立证据。
