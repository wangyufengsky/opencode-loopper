...
- Designer 页面每 1.2 秒读取有界活动投影：交互设计师可展示思考、增量文字和普通/MCP 工具调用，Router/Decomposer/Compiler/Reviewer/repair/finalizer 等结构化角色只展示工具活动和权威步骤，禁止泄露原始规划 JSON。断线保留最后活动并重连，活动投影不得推进生命周期。
