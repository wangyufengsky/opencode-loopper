...
6. 确认生成新的可执行 JAR：
   ...
   ```bash
   test -s target/opencode-loopper-0.2.58.jar
   jar tf target/opencode-loopper-0.2.58.jar \
     | rg 'BOOT-INF/classes/static/(index.html|assets/)'
   shasum -a 256 target/opencode-loopper-0.2.58.jar
   ```
...
- Maven 项目版本：`0.2.58`。
- 正式产物：`target/opencode-loopper-0.2.58.jar`。
...
```bash
JAR=target/opencode-loopper-0.2.58.jar
test -s "$JAR"
