...
6. 确认生成新的可执行 JAR：
   ...
   ```bash
   test -s target/opencode-loopper-0.1.56.jar
   jar tf target/opencode-loopper-0.1.56.jar \
     | rg 'BOOT-INF/classes/static/(index.html|assets/)'
   shasum -a 256 target/opencode-loopper-0.1.56.jar
   ```
...
- Maven 项目版本：`0.1.56`。
- 正式产物：`target/opencode-loopper-0.1.56.jar`。
...
```bash
JAR=target/opencode-loopper-0.1.56.jar
test -s "$JAR"
...
| --- | --- | --- | --- |
| 2026-08-14 | 稳定跨 Git 版本的独立文本合并夹具并交付 0.1.56 | 自动合并测试的源/任务修改之间保留充足未改动行，避免 xdiff 版本差异把相邻修改折叠为同一冲突 hunk；真实相邻修改继续进入人工解决；同步 README、架构与本公约正文 | 注入 `core.autocrlf=true`/`core.safecrlf=warn` 的聚焦 Java 56/56、Vitest 149/149；`./scripts/verify.sh` 通过：Java 366 个（0 失败、0 错误、1 个 Windows 专属用例在 macOS 跳过），Vitest 149/149；JAR `target/opencode-loopper-0.1.56.jar`（262946385 bytes）内含 100 个前端静态资源，SHA-256 `fd370d2031ca10167f1bb9371f5aa9a58fe1340f355b66b8cd04249b09575cf0`；发布目标：`v0.1.56` |
