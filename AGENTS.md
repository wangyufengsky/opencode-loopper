...
当前交付版本：`0.3.92`。实际版本由 `pom.xml` 持有，发布引用用脚本同步。
...
代码与构建变更默认以完整 JAR 及内置 JDK 21 的六个平台包为交付，除非用户另有要求。文档若进入运行提示、生成规则或打包内容，按实际行为影响分类。快速测试使用 `target/backend-dev`，没有正式静态资源，不是完整门禁或成品。

完整交付：同步一个未使用且递增的版本 → `./scripts/verify.sh` → 检查正式 JAR、六个平台包、静态资源和 SHA-256 → 更新交付记录 → 本地提交。只回填测试数、哈希和结果的记录不改变产物，不触发递归打包。聚焦失败先修复；完整候选构建后若可执行交付内容再变，重新分配版本。失败输出必须保留，禁止伪造成功。
