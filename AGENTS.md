...
当前交付版本：`0.3.87`。实际版本由 `pom.xml` 持有，发布引用用脚本同步。
...
代码与构建变更默认以完整 JAR 为交付，除非用户另有要求。文档若进入运行提示、生成规则或打包内容，按实际行为影响分类。快速测试使用 `target/backend-dev`，没有正式静态资源，不是完整门禁或成品。
...
| 安装、配置、运维 | [README](README.md) | `application.yml`、两平台启动脚本 |
