...
`OPENCODE_BASE_URL` 优先；否则只能从当前 `opencode serve --port ...`
进程提取候选端口并用 `/global/health` 的 `healthy=true` 验真。没有
可复用实例时使用 `auto` 模式，由 Loopper 在动态 loopback 端口启动
受管进程；不得把任意监听端口推断为 OpenCode。
